// ═══════════════════════════════════════════════════════════════
// STRUCTBOARD — Brochure Parser v1.2
// v1.2: fix underlyings [object Object] + prompt clarification
...
(function() {
  ...
  var PARSER_PROMPT = [
    ...
    '',
    'CAPITAL:',
    '- "Protection du capital : Non" → protected=false. MÊME si "100% du Capital Initial" apparaît.',
    '- "capital garanti" / "protection totale" → protected=true, level=100',
    '- Barrière capital = seuil de PERTE. Barrière coupon = seuil de VERSEMENT.',
    ...
    '',
    'UNDERLYING TYPE (valeurs exactes): "single-index", "single-stock", "worst-of", "basket", "pairs", "none"',
    '',
    'UNDERLYINGS: tableau de STRINGS simples. Ex: ["Eurostoxx 50"] ou ["Indice Solactive GDX EUR AR 5%"].',
    'NE PAS retourner d\'objets dans underlyings. Uniquement des strings.',
    '',
  ...
  function _normalizeStringArray(arr) {
    ...
    return arr.map(function(item) {
      ...
      if (item && typeof item === 'object') {
        // Claude sometimes returns {name: "...", ticker: "..."} or {name: "...", isin: "..."}
        return item.name || item.ticker || item.label || item.isin || JSON.stringify(item);
      }
      return String(item);
    }).filter(Boolean);
  }
  ...
  function _postProcess(data) {
    ...
    // ── CRITICAL: normalize underlyings to strings ──
    ...
    if (UND_MAP[ut]) {
      ...
      if (undCount === 0) data.underlyingType = 'none';
        ...
        data.underlyingType = (/indice|index|euro|cac|s&p|solactive|stoxx/.test(name)) ? 'single-index' : 'single-stock';
      } else {
    ...
    if (c.rate && c.rate > 12) {
      if (c.frequency === 'semestriel' && c.rate > 15) {
        ...
        if (possible >= 2 && possible <= 12) c.rate = possible;
      } else if (c.frequency === 'annuel' && c.rate > 15) {
        var yrs = data.maturityYears || 5;
        var possibleAnn = c.rate / yrs;
        if (possibleAnn >= 2 && possibleAnn <= 12) c.rate = possibleAnn;
      }
    }
    ...
    // Capital garanti false positive
  ...
  function _buildProduct() {
    ...
    return {
      ...
      coupon: {
        rate: _gv('bp-rate'), rateIfCalled: _gv('bp-rateIfCalled'), rateIfMaturity: _gv('bp-rateIfMaturity'),
      ...
      earlyRedemption: {
        possible: _gv('bp-erpossible') || false, type: _gv('bp-ertype') || 'none',
  ...
  async function _analyzePDF(file) {
    ...
    try {
      ...
      var resp = await fetch(CONFIG.AI_ENDPOINT, {
        ...
        body: JSON.stringify({
          model: 'claude-sonnet-4-20250514', max_tokens: 3000, system: PARSER_PROMPT,
      ...
      _error = e.message; _phase = 'upload'; _render();
    }
  }
  ...
  function _copyJSON() {
    ...
    navigator.clipboard.writeText(JSON.stringify(product, null, 2)).then(function() {
      showToast('✅ JSON copié', 'success');
    });
  }

  function _render() {
    var container = document.getElementById('brochure-parser-root'); if (!container) return;
  ...
  function _renderUpload() {
    return '<div class="bp-center"><div class="bp-upload-header">' +
      ...
      '<div class="bp-upload-sub">Upload → Claude analyse → Tu valides → Ajout direct dans StructBoard</div></div>' +
  ...
  function _bindUploadEvents() {
    var dz = document.getElementById('bp-dropzone');
    var fi = document.getElementById('bp-file-input');
    if (!dz || !fi) return;
    ...
    dz.addEventListener('drop', function(e) {
      ...
      if (f && f.name.toLowerCase().endsWith('.pdf')) _analyzePDF(f);
  ...
  function _renderLoading() {
    return '<div class="bp-center" style="min-height:400px;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:16px">' +
      ...
      '<div style="font-size:11px;color:var(--text-muted)">Claude lit la brochure et extrait les données (10-30s)</div></div>';
  }
  ...
  function _renderReview() {
    ...
    var html = '<div class="bp-review"><div class="bp-review-header">' +
      ...
      '<button class="btn primary" id="bp-btn-add">🚀 Ajouter à StructBoard</button></div></div>';
    ...
    html += _section('SOUS-JACENTS', '📊', 'var(--purple)',
      ...
      '<div class="bp-hint">Séparés par virgule</div></div>' +
    ...
    html += _section('MÉCANISME & RISQUES', '⚙️', 'var(--text-dim)',
      '<div class="bp-field bp-span-full"><div class="bp-label">Description du mécanisme</div>' +
      ...
      _inp('bp-risks', (d.risks || []).join(' · '), 'text', 'Séparés par ·') + '</div>');
  ...
  function _bindReviewEvents() {
    ...
    if (btnReset) btnReset.addEventListener('click', function() { _phase = 'upload'; _data = null; _fileName = ''; _error = ''; _render(); });
    if (btnCopy) btnCopy.addEventListener('click', _copyJSON);
    if (btnAdd) btnAdd.addEventListener('click', function() { _phase = 'bank-select'; _render(); });
  }
  ...
  function _renderBankSelect() {
    ...
    var autoBank = '';
    if (emitter) { var emLower = emitter.toLowerCase(); BANKS_LIST.forEach(function(b) { if (emLower.indexOf(b.name.toLowerCase()) >= 0 || emLower.indexOf(b.id) >= 0) autoBank = b.id; }); }
    if (autoBank && !_selectedBank) _selectedBank = autoBank;

    var html = '<div class="bp-center" style="max-width:560px">' +
      '<div style="text-align:center;margin-bottom:24px"><div style="font-size:16px;font-weight:700;color:var(--text-bright);margin-bottom:6px">Ajouter à StructBoard</div>' +
      '<div style="font-size:12px;color:var(--text-muted)">' + esc(productName) + '</div></div>' +
    ...
    html += '</div></div><div class="bp-field" style="margin-bottom:24px"><div class="bp-label">Montant investi (€) — optionnel</div>' +
      ...
      '<button class="btn primary" id="bp-bank-confirm"' + (!_selectedBank ? ' disabled' : '') + '>✅ Confirmer et ajouter</button></div></div>';
    return html;
  }

  function _bindBankSelectEvents() {
    document.querySelectorAll('.bank-select-btn').forEach(function(btn) { btn.addEventListener('click', function() { _selectedBank = btn.dataset.bank; _render(); }); });
    var ai = document.getElementById('bp-amount'); if (ai) ai.addEventListener('input', function() { _investedAmount = ai.value; });
    var bb = document.getElementById('bp-bank-back'); if (bb) bb.addEventListener('click', function() { _phase = 'review'; _render(); });
    var bc = document.getElementById('bp-bank-confirm'); if (bc) bc.addEventListener('click', _addToStructBoard);
  }
  ...
  console.log('[StructBoard] Brochure Parser v1.2 loaded');
})();
