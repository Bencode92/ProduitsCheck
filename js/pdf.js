// ═══════════════════════════════════════════════════════════════
// STRUCTBOARD — PDF Extraction & AI Parsing V7.8
...
class AIParser {
  ...
  async parseBrochure(rawText) {
    ...
    try {
      ...
      // This catches cases where AI missed it or classified as autocall
      ...
      for (var bk = 0; bk < basketKeywords.length; bk++) {
        if (txtLower.indexOf(basketKeywords[bk].toLowerCase()) >= 0) {
          ...
          console.log('[parseBrochure V7.8] Basket detected via rawText keyword: "' + basketKeywords[bk] + '"');
          break;
        }
      }
      // Also check: has "niveau du panier" but NOT "worst" or "pire"
      if (!isBasketFromText && txtLower.indexOf('niveau du panier') >= 0 && txtLower.indexOf('worst') < 0 && txtLower.indexOf('pire') < 0) {
        isBasketFromText = true;
        console.log('[parseBrochure V7.8] Basket detected via "niveau du panier" without worst-of');
      }

      if (isBasketFromText) {
        // Override structureType to basket (even if AI said autocall)
        if (!parsed.structureType || parsed.structureType === 'autocall') {
          parsed.structureType = 'basket';
          console.log('[parseBrochure V7.8] structureType overridden to "basket"');
        }
        // Set underlyingType
        if (!parsed.underlyingType || parsed.underlyingType === 'worst-of') {
          parsed.underlyingType = 'basket';
        }
        // Mark it explicitly
        parsed._isBasket = true;
      }

      // V7.8: Store rawText snippet for downstream basket-detection-v2
      // (first 3000 chars is enough for keyword detection without bloating JSON)
      ...
      console.log('[parseBrochure V7.8] type:', parsed.structureType, '| basket:', parsed._isBasket || false, '| autocall:', parsed.earlyRedemption?.possible || false, '| barrier:', parsed.capitalProtection?.barrier || 'none', '| barrierCoupon:', parsed.capitalProtection?.barrierCoupon || 'none', '| coupon:', parsed.coupon?.rate, '| freq:', parsed.coupon?.frequency, '| model:', _getAIModel());
      return parsed;
  ...
  async _callAI(prompt, maxTokens) {
    ...
    if (!res.ok) {
      if (typeof CONFIG !== 'undefined' && CONFIG.AI_MODEL_FALLBACK && model !== CONFIG.AI_MODEL_FALLBACK) {
        console.warn('[pdf.js V7.8] ' + model + ' failed, falling back to ' + CONFIG.AI_MODEL_FALLBACK);
  ...
  async _callAIWithHistory(systemPrompt, messages) {
    ...
    if (!res.ok) {
      if (typeof CONFIG !== 'undefined' && CONFIG.AI_MODEL_FALLBACK && model !== CONFIG.AI_MODEL_FALLBACK) {
        console.warn('[pdf.js V7.8] ' + model + ' chat failed, falling back to ' + CONFIG.AI_MODEL_FALLBACK);
