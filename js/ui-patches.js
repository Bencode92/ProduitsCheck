// ═══ PATCHES V12d — AI Summary: reliable button + GitHub persistence ═══

let _pendingProduct = null;
let _cachedAISummary = null; // Loaded from GitHub on init
...
// ─── Load saved AI summary from GitHub ──────────────────────
async function _loadAISummary() {
  try {
    const resp = await fetch(`https://raw.githubusercontent.com/${CONFIG.REPO_OWNER}/${CONFIG.REPO_NAME}/${CONFIG.BRANCH}/${CONFIG.DATA_PATH}/ai-summary.json?t=${Date.now()}`);
    if (resp.ok) { _cachedAISummary = await resp.json(); }
  } catch (e) { /* no saved summary */ }
}
// Load on startup
_loadAISummary();
...
function _renderSavedSummary() {
  if (!_cachedAISummary?.text) return '';
  const html = _formatSummaryHTML(_cachedAISummary.text);
  return `<div style="font-size:13px;line-height:1.6;color:var(--text)">${html}</div>
    <div style="font-size:10px;color:var(--text-dim);margin-top:12px;text-align:right">Généré le ${_cachedAISummary.date || '?'} à ${_cachedAISummary.time || '?'}</div>`;
}

// ═══ AI PORTFOLIO SUMMARY — Generate + Save to GitHub ═══════
async function generatePortfolioSummary() {
  ...
  const productsData = portfolio.map(p => {
    ...
    return {
      ...
      tracking: s ? { variation: (s.variation >= 0 ? '+' : '') + s.variation.toFixed(1) + '%', coupon_ok: s.couponOK, marge_barriere: s.margeRestante ? s.margeRestante.toFixed(1) + '%' : 'N/A', date: s.date } : null
    };
  });
  const totalInvested = portfolio.reduce((s, p) => s + (parseFloat(p.investedAmount) || 0), 0);
  const totalYield = portfolio.reduce((s, p) => { const r = typeof getAnnualizedRate === 'function' ? getAnnualizedRate(p) : (parseFloat(p.coupon?.rate) || 0); return s + Math.round((parseFloat(p.investedAmount) || 0) * r / 100); }, 0);
...
Sois direct, pas de jargon inutile. Utilise les chiffres du portefeuille.`;
  ...
  try {
    ...
    const summaryData = {
      ...
      productsCount: portfolio.length,
      totalInvested: totalInvested
    };
    _cachedAISummary = summaryData;

    // Save to GitHub
    try {
      await github.writeFile(`${CONFIG.DATA_PATH}/ai-summary.json`, summaryData, `[StructBoard] AI Summary ${summaryData.date}`);
    } catch (e) { console.warn('Could not save AI summary to GitHub:', e); }

    box.innerHTML = _renderSavedSummary();
    showToast('Résumé IA généré et sauvegardé', 'success');
  } catch (e) {
    box.innerHTML = `<div style="color:var(--red);padding:12px">Erreur: ${e.message}</div>`;
  }
  btn.disabled = false; btn.innerHTML = '🤖 Renouveler le résumé IA';
}

// ═══ Upload / Manual / Integrate overrides (unchanged) ══════
...
renderProductCard = function(product, context) {
  ...
  if (product.entity) { const ei = MY_ENTITIES.find(e => e.id === product.entity); if (ei) { const badge = `<div class="product-card-bank" style="color:${ei.color};border-color:${ei.color}33;background:${ei.color}12;margin-left:4px">${ei.icon} ${ei.name}</div>`; const he = html.indexOf('</div></div>'); if (he >= 0) html = html.substring(0, he) + badge + html.substring(he); } }
  let extra = '';
...
// ═══ renderDashboard — RELIABLE AI button injection ═════════
...
renderDashboard = function(container, state) {
  ...
  if (typeof getPortfolioAlerts === 'function') {
    ...
    if (alerts.length > 0) {
      ...
      // Find portfolio section and inject before it
      ...
      if (portfolioSection) {
        ...
        portfolioSection.before(ta);
      }
    }
  }

  // ─── AI SUMMARY — inject into portfolio section header ────
  if (active.length > 0) {
    const sections = container.querySelectorAll('.section');
    const portfolioSection = sections[0];
    if (portfolioSection) {
      ...
      if (sectionHeader) {
        // Add AI button next to "+ Ajouter"
        const aiBtn = document.createElement('button');
        aiBtn.id = 'ai-summary-btn';
        aiBtn.className = 'btn ai-glow';
        aiBtn.style.cssText = 'white-space:nowrap;margin-right:8px';
        aiBtn.innerHTML = _cachedAISummary ? '🤖 Renouveler le résumé IA' : '🤖 Résumé IA';
        aiBtn.onclick = generatePortfolioSummary;
        sectionHeader.insertBefore(aiBtn, sectionHeader.querySelector('.btn'));
      }
      // AI summary box — show saved or empty
      const aiBox = document.createElement('div');
      aiBox.id = 'ai-summary-box';
      aiBox.style.cssText = `${_cachedAISummary ? '' : 'display:none;'}background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:16px;margin-bottom:16px`;
      aiBox.innerHTML = _cachedAISummary ? _renderSavedSummary() : '';
      // Insert AI box BEFORE portfolio grid
      const grid = portfolioSection.querySelector('.portfolio-grid') || portfolioSection.querySelector('.empty-state');
      if (grid) grid.before(aiBox);
      else portfolioSection.appendChild(aiBox);
    }
  }
...
renderProductSheet = function(container, state) {
  ...
  if (sidebar) {
    ...
    if (!p.archived) {
      ...
      if (_isInPortfolio(p) && typeof showArchiveModal === 'function') { const archBtn = document.createElement('button'); archBtn.className = 'btn lg'; archBtn.style.cssText = 'width:100%;background:rgba(148,163,184,0.1);border:1px solid #94A3B8;color:#94A3B8'; archBtn.innerHTML = '\ud83d\udce6 Archiver (produit terminé)'; archBtn.onclick = () => showArchiveModal(); sidebar.appendChild(archBtn); }
    }
  }
  if (_isInPortfolio(p) && !p.archived) { const notice = container.querySelector('.integrated-notice'); if (notice) { const rd = p.subscriptionDate ? new Date(p.subscriptionDate).toLocaleDateString('fr-FR') : formatDate(p.addedDate); const el = p.entity ? (MY_ENTITIES.find(e => e.id === p.entity)?.name||'') : ''; const bl = p.bankId ? (BANKS_LIST.find(b => b.id === p.bankId)?.name||p.bankId) : ''; notice.innerHTML = `\u2705 Intégré le ${rd}${el ? '<br>\ud83c\udfe2 '+el : ''}${bl ? '<br>\ud83c\udfe6 '+bl : ''}<br>Montant: ${formatNumber(p.investedAmount)}\u20ac${p.integrationNotes ? '<br><span style="color:var(--text-dim);font-size:11px">'+p.integrationNotes+'</span>' : ''}`; } }
};
