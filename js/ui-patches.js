// ═══ PATCHES V12h — Deep Analysis injection in product sheet ═══
...
function _buildLiquidityBanner() {
  ...
  return `<div style="background:var(--bg-elevated);border:1px solid var(--border);border-radius:var(--radius);padding:16px;margin-bottom:16px">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
      ${cashBC > 0 ? `<div style="background:rgba(6,214,160,0.06);border:1px solid rgba(6,214,160,0.2);border-radius:var(--radius-sm);padding:12px"><div style="display:flex;justify-content:space-between;align-items:center"><span style="font-size:12px;font-weight:600;color:#06D6A0">🏢 ByCam</span><span style="font-size:18px;font-weight:800;color:#06D6A0;font-family:var(--mono)">${formatNumber(cashBC)}€</span></div>${structBC > 0 ? '<div style="font-size:10px;color:var(--text-dim);margin-top:4px">Déjà en structurés : ' + formatNumber(structBC) + '€</div>' : ''}</div>` : ''}
      ${cashCam > 0 ? `<div style="background:rgba(131,56,236,0.06);border:1px solid rgba(131,56,236,0.2);border-radius:var(--radius-sm);padding:12px"><div style="display:flex;justify-content:space-between;align-items:center"><span style="font-size:12px;font-weight:600;color:#8338EC">🦎 Caméleons</span><span style="font-size:18px;font-weight:800;color:#8338EC;font-family:var(--mono)">${formatNumber(cashCam)}€</span></div>${structCam > 0 ? '<div style="font-size:10px;color:var(--text-dim);margin-top:4px">Déjà en structurés : ' + formatNumber(structCam) + '€</div>' : ''}</div>` : ''}
    </div>
    <div style="margin-top:10px;display:flex;gap:4px;height:6px;border-radius:3px;overflow:hidden">
      ${catInvested > 0 ? '<div style="flex:' + catInvested + ';background:var(--green)"></div>' : ''}
      ${structInvested > 0 ? '<div style="flex:' + structInvested + ';background:var(--purple)"></div>' : ''}
      <div style="flex:${placable};background:var(--cyan)"></div>
    </div>
...
function _buildPortfolioSummaryHTML(active, aiInsights) {
  ...
  const rows = active.map(p => {
    ...
    const amount = parseFloat(p.investedAmount)||0; const yieldAmt = Math.round(amount * annRate / 100);
    const s = typeof getTrackingStatus==='function' ? getTrackingStatus(p) : null;
    const variation = s ? s.variation : null; const couponOK = s ? s.couponOK : true;
    const margeRestante = s?.margeRestante ?? null; const barrier = parseFloat(p.capitalProtection?.barrier)||0;
  ...
  const couponsOK = rows.filter(r => r.couponOK).length; const couponsLost = rows.filter(r => !r.couponOK).length;
  ...
  const bankCount = Object.keys(bankMap).length; const diversOK = topBankPct <= 40;
  ...
  return `<div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);overflow:hidden">
    ...
    <table style="width:100%;font-size:12px;border-collapse:collapse"><thead><tr style="background:var(--bg)"><th style="padding:8px 12px;text-align:left;color:var(--text-dim);font-size:10px;text-transform:uppercase;font-weight:600">Produit</th><th style="padding:8px 8px;text-align:right;color:var(--text-dim);font-size:10px;text-transform:uppercase;font-weight:600">Montant</th><th style="padding:8px 8px;text-align:right;color:var(--text-dim);font-size:10px;text-transform:uppercase;font-weight:600">Taux</th><th style="padding:8px 8px;text-align:right;color:var(--text-dim);font-size:10px;text-transform:uppercase;font-weight:600">Rdt/an</th><th style="padding:8px 8px;text-align:center;color:var(--text-dim);font-size:10px;text-transform:uppercase;font-weight:600">Variation</th><th style="padding:8px 8px;text-align:center;color:var(--text-dim);font-size:10px;text-transform:uppercase;font-weight:600">Statut</th></tr></thead>
      <tbody>${rows.map(r => { const varStr = r.variation !== null ? ((r.variation>=0?'+':'')+r.variation.toFixed(1)+'%') : '—'; const varColor = r.variation===null?'var(--text-dim)':r.variation>=0?'var(--green)':r.variation>-20?'var(--orange)':'var(--red)';
        return `<tr style="border-top:1px solid var(--border);cursor:pointer" onclick="app.openProduct(app._findProduct('${r.id}','${r.bankId}'))"><td style="padding:8px 12px"><div style="font-weight:600;color:var(--text-bright)">${r.name}</div><div style="font-size:10px;color:var(--text-dim)">${r.bank}${r.entity?' · <span style=color:'+r.entity.color+'>'+r.entity.icon+r.entity.name+'</span>':''}</div></td><td style="padding:8px;text-align:right;font-family:var(--mono)">${formatNumber(r.amount)}€</td><td style="padding:8px;text-align:right;font-family:var(--mono);color:var(--green)">${r.annRate.toFixed(2)}%</td><td style="padding:8px;text-align:right;font-family:var(--mono);font-weight:600;color:var(--green)">${formatNumber(r.yieldAmt)}€</td><td style="padding:8px;text-align:center;font-family:var(--mono);font-weight:600;color:${varColor}">${varStr}</td><td style="padding:8px;text-align:center"><span style="display:inline-block;padding:2px 8px;border-radius:10px;font-size:10px;font-weight:600;background:${r.riskColor}22;color:${r.riskColor}">${r.riskIcon} ${r.riskLevel}</span></td></tr>`;
      }).join('')}
...
async function generatePortfolioSummary() {
  const btn = document.getElementById('ai-summary-btn'), box = document.getElementById('ai-summary-box');
  if (!btn||!box) return; btn.disabled = true; btn.innerHTML = '⏳ Analyse...';
  ...
  const productsData = active.map(p => { const annRate = typeof getAnnualizedRate==='function' ? getAnnualizedRate(p) : (parseFloat(p.coupon?.rate)||0); const s = typeof getTrackingStatus==='function' ? getTrackingStatus(p) : null; return `${(p.name||'').substring(0,30)}: ${formatNumber(p.investedAmount)}€, ${annRate}%/an${s?', var '+((s.variation>=0?'+':'')+s.variation.toFixed(1))+'%'+(!s.couponOK?' COUPON PERDU':''):''}`;}).join('\n');
  try {
    const resp = await fetch(CONFIG.AI_ENDPOINT, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 400, messages: [{ role: 'user', content: `Portefeuille structurés:\n${productsData}\n\n3 bullet points courts:\n- 1 constat clé\n- 1 risque principal\n- 1 action recommandée\nFormat: "▸ texte". Pas de titre.`}]})});
    ...
    box.innerHTML = _buildPortfolioSummaryHTML(active, insights + `<div style="font-size:9px;color:var(--text-dim);text-align:right;margin-top:4px">${_cachedAISummary.date} ${_cachedAISummary.time}</div>`);
    try { await github.writeFile(`${CONFIG.DATA_PATH}/ai-summary.json`, _cachedAISummary, `[StructBoard] AI Summary`); } catch(e){}
    showToast('OK','success');
  } catch(e) { box.innerHTML = _buildPortfolioSummaryHTML(active, `<span style="color:var(--red)">${e.message}</span>`); }
  btn.disabled = false; btn.innerHTML = '🤖 Actualiser';
}
function _renderSavedSummary() { const active = (app.state.portfolio||[]).filter(p => !p.archived); if (!active.length) return ''; const insights = _cachedAISummary?.insights ? _cachedAISummary.insights + `<div style="font-size:9px;color:var(--text-dim);text-align:right;margin-top:4px">${_cachedAISummary.date||'?'} ${_cachedAISummary.time||'?'}</div>` : null; return _buildPortfolioSummaryHTML(active, insights); }
...
const _origRenderDashboard=renderDashboard;renderDashboard=function(container,state){
  ...
  if (liqBanner) { const el = container.querySelector('.stats-row'); if (el) el.insertAdjacentHTML('afterend', liqBanner); else container.insertAdjacentHTML('afterbegin', liqBanner); }
  ...
  if(active.length>0){const secs=container.querySelectorAll('.section');const ps=secs[0];if(ps){const sh=ps.querySelector('.section-header');if(sh){const aiBtn=document.createElement('button');aiBtn.id='ai-summary-btn';aiBtn.className='btn ai-glow';aiBtn.style.cssText='white-space:nowrap;margin-right:8px';aiBtn.innerHTML=_cachedAISummary?'🤖 Actualiser':'🤖 Résumé IA';aiBtn.onclick=generatePortfolioSummary;sh.insertBefore(aiBtn,sh.querySelector('.btn'));}const aiBox=document.createElement('div');aiBox.id='ai-summary-box';const hasSaved=!!_cachedAISummary?.insights;aiBox.style.cssText=`${hasSaved?'':'display:none;'}margin-bottom:16px`;aiBox.innerHTML=hasSaved?_renderSavedSummary():'';const grid=ps.querySelector('.portfolio-grid')||ps.querySelector('.empty-state');if(grid)grid.before(aiBox);}}
...
// ═══ renderProductSheet — WITH DEEP ANALYSIS ═════════════
const _origRenderProductSheet=renderProductSheet;renderProductSheet=function(container,state){
  ...
  // ═══ DEEP ANALYSIS INJECTION ═══
  if (typeof injectDeepAnalysis === 'function') {
    injectDeepAnalysis(container, p);
  }
