// STRUCTBOARD — Proposal Grader v2.2 — Self-exclusion fix for portfolio grading
...
function _annualizeCoupon(rawRate, couponObj, productName) {
    ...
    if (detection.mult > 1 && annual > 30) return { annual: rawRate, raw: rawRate, mult: 1, freq: 'annuel', source: 'sanity-cap' };
...
function _graderNormalize(product) {
    ...
    return {
        id: p.id || '', // Keep ID for self-exclusion
...
// ═══════════════════════════════════════════════════════════════
// PORTFOLIO CONTEXT — with self-exclusion
// ═══════════════════════════════════════════════════════════════
// When grading a product already in portfolio, exclude it from
// the portfolio stats so it doesn't penalize itself for overlap
// or concentration.

function _analyzePortfolioContext(product, portfolio) {
    // SELF-EXCLUSION: filter out the product being graded
    var productId = product.id || '';
    var products = (portfolio || []).filter(function(p) {
        return p.id !== productId;
    });

    var totalAmount = products.reduce(function(s, p) { return s + (parseFloat(p.investedAmount || p.nominal) || 0); }, 0);

    // Issuer concentration (excluding self)
    var issuerAmounts = {};
    products.forEach(function(p) {
        var k = (p.bankId || '?').toUpperCase();
        issuerAmounts[k] = (issuerAmounts[k] || 0) + (parseFloat(p.investedAmount || p.nominal) || 0);
    });
    var issuerConc = {};
    Object.keys(issuerAmounts).forEach(function(k) {
        issuerConc[k] = totalAmount > 0 ? issuerAmounts[k] / totalAmount : 0;
    });

    // Type counts (excluding self)
    var typeCounts = {};
    products.forEach(function(p) {
        var t = (p.type || 'autre').toUpperCase();
        typeCounts[t] = (typeCounts[t] || 0) + 1;
    });

    // Existing underlyings (excluding self)
    var existingUnds = {};
    products.forEach(function(p) {
        _normalizeUnderlyings(p.underlyings || (p.aiParsed && p.aiParsed.underlyings) || []).forEach(function(u) {
            existingUnds[u.toUpperCase()] = 1;
        });
    });

    // Average coupon (excluding self)
    var coupons = products.map(function(p) {
        var c = p.coupon;
        return typeof c === 'number' ? c : parseFloat(c && c.rate || 0);
    }).filter(function(c) { return c > 0; });
    var avgCoupon = coupons.length > 0 ? Math.round((coupons.reduce(function(a, b) { return a + b; }, 0) / coupons.length) * 100) / 100 : 0;

    // Overlap (product underlyings vs rest of portfolio)
    var overlap = product.underlyings.map(function(u) { return u.toUpperCase(); }).filter(function(u) { return existingUnds[u]; });

    // How many products were excluded (for logging)
    var excluded = (portfolio || []).length - products.length;

    return {
        available: true,
        totalProducts: products.length,
        totalAmount: totalAmount,
        issuerConcentration: issuerConc,
        currentIssuerPct: issuerConc[(product.bankId || '').toUpperCase()] || 0,
        typeCounts: typeCounts,
        existingUnderlyings: Object.keys(existingUnds),
        overlappingUnderlyings: overlap,
        avgCoupon: avgCoupon,
        selfExcluded: excluded > 0
    };
}
...
function _buildUserPrompt(context){
    ...
    if(portfolio.available){
        p+='\n## PORTEFEUILLE EXISTANT'+(portfolio.selfExcluded?' (hors produit analysé)':'')+' — '+portfolio.totalProducts+' produits, '+formatNumber(portfolio.totalAmount)+'€\n';
        ...
        p+='- Overlap sous-jacents: '+(portfolio.overlappingUnderlyings.length>0?portfolio.overlappingUnderlyings.join(', '):'aucun')+'\n';
    }
...
async function gradeProposal(product){var t0=Date.now();var ctx=await _collectGradingContext(product);var kill=_checkKillCriteria(ctx.product,ctx.portfolio,ctx.cat);if(kill.killed){var r={grade:'F',score:0,killCriteria:{triggered:true,reasons:kill.reasons},pillars:{adjustedReturn:{score:null},underlyingQuality:{score:null},portfolioFit:{score:null},riskPremium:{score:null}},verdict:'Rejet automatique: '+kill.reasons[0],keyRisks:kill.reasons,scenarios:null,metadata:{gradedAt:new Date().toISOString(),durationMs:Date.now()-t0,aiUsed:false,version:'2.2',marketDataAvailable:ctx.market.available}};product.grading=r;return r}var ai;try{ai=await _callClaude(ctx)}catch(e){console.error('[Grader] Claude:',e);ai=_localFallback(ctx)}var result=_normalizeResult(ai);result.killCriteria=result.killCriteria||{triggered:false,reasons:[]};result.metadata={gradedAt:new Date().toISOString(),durationMs:Date.now()-t0,aiUsed:true,version:'2.2',marketDataAvailable:ctx.market.available,catBenchmark:ctx.cat.bestRate,couponAnnualized:ctx.product.coupon,couponRaw:ctx.product.couponRaw,couponMultiplier:ctx.product.couponMultiplier,couponFrequency:ctx.product.couponFrequency,selfExcluded:ctx.portfolio.selfExcluded||false};product.grading=result;return result}
...
function renderGradingSection(grading){
    ...
    if(g.metadata)h+='<div style="font-size:10px;color:var(--text-muted);opacity:0.5;margin-top:8px">Grad\u00e9 le '+new Date(g.metadata.gradedAt).toLocaleDateString('fr-FR')+' '+(g.metadata.aiUsed?'\u00b7 Claude IA':'\u00b7 Local')+' \u00b7 '+g.metadata.durationMs+'ms '+(g.metadata.marketDataAvailable?'\u00b7 Donn\u00e9es march\u00e9 \u2713':'')+(g.metadata.selfExcluded?' \u00b7 Self-excluded \u2713':'')+'</div>';
    h+='</div>';return h;
}

window.ProposalGrader={grade:gradeProposal,gradeBatch:gradeProposalsBatch,renderBadge:renderGradeBadge,renderSection:renderGradingSection,checkKillCriteria:_checkKillCriteria,normalize:_graderNormalize,config:GRADING_CONFIG,annualizeCoupon:_annualizeCoupon,detectFrequency:_detectCouponFrequency,version:'2.2'};
console.log('[StructBoard] ProposalGrader v2.2 \u2014 self-exclusion + coupon annualization');
