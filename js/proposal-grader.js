...
function _computeP3(p, portfolio, isInPf) {
    ...
    for (var i = 0; i < overlap.length; i++) { s -= 10 + (i * 5); }
...
function _buildSystemPrompt(isInPf, productType) {
    ...
    if (productType === 'fixed-rate-callable') {
        base += "\n⚠ PRODUIT TAUX FIXE CALLABLE :\n- Pas de sous-jacent action — c'est une OBLIGATION callable\n- Le risque principal est le CRÉDIT ÉMETTEUR + le risque de RÉINVESTISSEMENT\n- Si rappelé tôt : bonus (5%/an au lieu de 4%/an) mais risque de réinvestir à taux plus bas\n- Si va à maturité : rendement garanti 4%/an\n- P2 = qualité crédit émetteur (notation, solidité)\n- Scénarios : optimiste=rappelé an 1-3 (5%/an), base=rappelé an 4-6 (5%/an), stress=va à maturité (4%/an), worst=défaut émetteur\n";
    }
