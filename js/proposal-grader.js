// STRUCTBOARD — Proposal Grader v4.3 — Expected maturity + beta penalty
// v4.3.1: Beta penalty in P2 for barrier products (systemic risk amplifier)
...
function _estimateExpectedMaturity(p) {
    ...
    if (p.worstOf && p.underlyings.length > 1) { probCallPerDate = Math.pow(probCallPerDate, Math.sqrt(p.underlyings.length)); }
    ...
    var expectedMat = 0; var probSurviving = 1.0;
    for (var i = 0; i < totalObs; i++) { var dateYear = firstObsYear + (i / obsPerYear); if (dateYear > matMax) break; var probCallHere = probSurviving * probCallPerDate; expectedMat += probCallHere * dateYear; probSurviving *= (1 - probCallPerDate); }
    expectedMat += probSurviving * matMax;
    expectedMat = Math.round(expectedMat * 10) / 10;
    return { expected: expectedMat, max: matMax, isEstimated: true, probCallPerDate: Math.round(probCallPerDate * 100), probReachMaturity: Math.round(probSurviving * 100), totalObsDates: totalObs };
}
...
function _computeP1(p) {
    ...
    if (!p.capitalProtection) {
        if (p.barrier > 0 && p.barrier < 100) { var barrierPenalty = Math.pow(Math.max(0, (p.barrier - 30) / 70), 1.5); s = s * (1 - barrierPenalty); }
        else { s -= 25; }
    }
...
// [v4.3.1] P2 with BETA PENALTY for barrier products
function _computeP2(p, market, productType) {
    ...
    // [v4.3.1] BETA PENALTY — systemic risk amplifier for barrier products
    // A high-beta stock amplifies market corrections toward the barrier
    // Market correction -15% → NVDA (beta 2.38) → -35.7% → barrier 60% nearly breached
    // Only applies when there IS a barrier (capital protected products don't care about beta)
    if (hasBarrier && wm.max_beta > 1.5) {
        s -= Math.round((wm.max_beta - 1.5) * 8);
        // NVDA beta 2.38 → -(2.38-1.5)*8 = -7 pts
        // ASML beta 1.1  → no penalty (< 1.5 threshold)
        // ENI beta 0.9   → no penalty
        // BMPS beta 1.2  → no penalty
        // TSLA beta 2.0  → -(2.0-1.5)*8 = -4 pts
    }
    return Math.max(0, Math.min(100, Math.round(s)));
}
...
async function gradeProposal(product) {
    ...
    var matInfo = p._maturityInfo || _estimateExpectedMaturity(p);
    var matNote = ''; if (matInfo.isEstimated) matNote = ' | Mat esp\u00e9r\u00e9e: ~' + matInfo.expected + 'a (max ' + matInfo.max + 'a, prob mat: ' + matInfo.probReachMaturity + '%)';
    // Beta info for P2 reasoning
    var betaNote = ''; if (ctx.market.worstMetrics && ctx.market.worstMetrics.max_beta > 1.5 && !p.capitalProtection && p.barrier > 0) { betaNote = ' | Beta max: ' + ctx.market.worstMetrics.max_beta + ' (\u03b2>1.5 penalty: -' + Math.round((ctx.market.worstMetrics.max_beta - 1.5) * 8) + ')'; }
...
function renderGradingSection(grading) {
    ...
    h += '<div style="font-size:10px;color:var(--text-muted);opacity:0.5;margin-top:8px">v4.3' + (g.metadata.aiUsed ? ' Hybride' : ' Local') + ' \u00b7 ' + new Date(g.metadata.gradedAt).toLocaleDateString('fr-FR') + ' \u00b7 ' + g.metadata.durationMs + 'ms \u00b7 CAT ' + (g.metadata.catBenchmark || '?') + '% (' + (g.metadata.catSource || '?') + ')' + (isPortfolio ? ' \u00b7 Portefeuille' : '') + (isCallable ? ' \u00b7 Callable' : '') + (g.metadata.hasBarrier ? ' \u00b7 P2 vol/DD 30%' : '') + (g.metadata.maxBeta && g.metadata.maxBeta > 1.5 ? ' \u00b7 \u03b2' + g.metadata.maxBeta : '') + '</div>';
    h += '</div>'; return h;
}
...
console.log('[StructBoard] ProposalGrader v4.3 \u2014 expected maturity + beta penalty');
