// STRUCTBOARD — Proposal Grader v4.0 — Post-audit corrections
// Implements: barrier in P1, conditional P2 weights, continuous P4,
//             CAT fallback 2.5%, kill coupon<1.5%, anti-momentum rule
...
const GRADING_CONFIG = {
    ...
    killCriteria: { maxWorstOfUnderlyings: 5, minCouponAnnualized: 1.5 }
};
...
// [AUDIT P0-1] CAT fallback updated from 3.0% to 2.5% (mars 2026 market)
// Dynamic loading from cat-market-rates.json and cat-deposits.json has priority
...
// ═══════════════════════════════════════════════════════════════
// DETERMINISTIC BASE SCORING — post-audit v4.0
// ═══════════════════════════════════════════════════════════════

// [AUDIT P1-1] P1 now integrates barrier distance for non-protected products
// Formula: base × (1 - max(0, (70 - barrier_pct) / 100))
// barrier 60% → -10%, barrier 40% → -30%, barrier 80% → 0% (deep = safe)
// Capital protected → no barrier penalty (replaces the flat -15)
function _computeP1(p) {
    var s = Math.min(100, p.coupon * 10);
    // Barrier-aware penalty (replaces flat -15 for non-protected)
    if (!p.capitalProtection) {
        if (p.barrier > 0 && p.barrier < 100) {
            // barrier = % of initial level. Higher = tighter = riskier
            // barrier 80% = stock drops 20% → knock-in. Very tight.
            // barrier 60% = stock drops 40% → moderate protection
            // barrier 40% = stock drops 60% → deep protection
            var barrierPenalty = Math.max(0, (p.barrier - 30) / 100); // 0 for deep, up to 0.5 for tight
            s = s * (1 - barrierPenalty);
        } else {
            // No barrier info but non-protected → flat penalty
            s -= 15;
        }
    }
...
// [AUDIT P1-2] P2 now uses conditional weights
// With barrier: vol/DD at 30% each (driver of knock-in risk)
// Capital protected: Buffett/Quality at 35% each (long-term quality)
function _computeP2(p, market) {
    ...
    var wBuffett, wQuality, wVol, wDD;
    if (hasBarrier) {
        // Barrier product: vol and DD are primary knock-in risk drivers
        wBuffett = 0.20; wQuality = 0.20; wVol = 0.30; wDD = 0.30;
    } else {
        // Capital protected: fundamental quality matters more
        wBuffett = 0.35; wQuality = 0.35; wVol = 0.15; wDD = 0.15;
    }

    var volComponent = Math.max(0, 100 - Math.max(0, (wm.max_volatility || 30) - 20) * 1.5);
    var ddComponent = Math.max(0, 100 - Math.max(0, (wm.max_drawdown || 30) - 25) * 1.2);
    var s = (wm.worst_buffett || 50) * wBuffett + (wm.worst_quality || 50) * wQuality + volComponent * wVol + ddComponent * wDD;

    // Correlation penalty for worst-of
...
function _computeP3(p, portfolio, isInPf) {
    ...
    var s = 70;
    // [AUDIT P2-3] Progressive overlap penalty: -10, -15, -20
    var overlap = portfolio.overlappingUnderlyings || [];
    for (var i = 0; i < overlap.length; i++) {
        s -= 10 + (i * 5); // 1st: -10, 2nd: -15, 3rd: -20
    }
...
// [AUDIT P1-3] P4 now continuous — no more step function
// Formula: min(100, max(5, spread_bps / 6))
// 150bps → 25, 300bps → 50, 450bps → 75, 600bps → 100
...
// ═══ KILL CRITERIA ═══
// [AUDIT] Added: coupon < 1.5% kill (clearly defective product)
function _checkKillCriteria(p, cat) {
    var reasons = [];
    if (p.worstOf && p.underlyings.length > GRADING_CONFIG.killCriteria.maxWorstOfUnderlyings)
        reasons.push('Worst-of ' + p.underlyings.length + ' sous-jacents (max ' + GRADING_CONFIG.killCriteria.maxWorstOfUnderlyings + ')');
    if (p.coupon > 0 && p.coupon < GRADING_CONFIG.killCriteria.minCouponAnnualized)
        reasons.push('Coupon annualisé ' + p.coupon + '% < seuil minimum ' + GRADING_CONFIG.killCriteria.minCouponAnnualized + '%');
...
// ═══════════════════════════════════════════════════════════════
// CLAUDE PROMPT — [AUDIT] anti-momentum rule added
// ═══════════════════════════════════════════════════════════════

function _buildSystemPrompt(isInPf) {
    return "Tu es un analyste de produits structurés expert.\n\nUn scoring de base a été calculé. Tu dois :\n1. AJUSTER chaque pilier de -15 à +15 points selon le CONTEXTE MARCHÉ\n2. JUSTIFIER chaque ajustement en 1 phrase\n3. VERDICT de 3-4 phrases avec données concrètes (Buffett score, vol, secteur)\n4. 2-3 RISQUES CLÉS\n5. SCÉNARIOS chiffrés en €\n\nRÈGLES D'AJUSTEMENT :\n- P1 (Rendement) : probabilité RÉELLE du coupon (distance barrière × vol du sous-jacent)\n- P2 (Qualité) : contexte macro, fondamentaux détaillés, secteur\n- P4 (Prime) : ajuste si spread trompeur (conditionnel vs garanti)\n" +
    (isInPf ? "- Ce produit est DÉJÀ en portefeuille. Mérites propres uniquement.\n" : "- P3 (Fit) : pertinence des sous-jacents pour le portefeuille\n") +
    "\n⚠ RÈGLE ANTI-MOMENTUM (OBLIGATOIRE) :\n" +
    "- NE JAMAIS utiliser perf YTD ou perf 1Y comme justification POSITIVE pour P2\n" +
    "- La performance passée N'EST PAS prédictive pour un structuré (horizon 3-12 ans)\n" +
    "- En P2, base tes ajustements UNIQUEMENT sur : fondamentaux (ROE, marge, dette), volatilité, drawdown, contexte sectoriel macro\n" +
    "- Tu PEUX utiliser perf négative comme signal de RISQUE (momentum négatif = warning)\n" +
...
// ═══ MAIN ORCHESTRATOR ═══
async function gradeProposal(product){var t0=Date.now();var ctx=await _collectContext(product);var p=ctx.product;var weights=ctx.isInPortfolio?GRADING_CONFIG.weightsPortfolio:GRADING_CONFIG.weightsProposal;var kill=_checkKillCriteria(p,ctx.cat);if(kill.killed){var r={grade:'F',score:0,killCriteria:{triggered:true,reasons:kill.reasons},pillars:{adjustedReturn:{score:null},underlyingQuality:{score:null},portfolioFit:{score:null},riskPremium:{score:null}},verdict:'Rejet: '+kill.reasons[0],keyRisks:kill.reasons,scenarios:null,metadata:{gradedAt:new Date().toISOString(),durationMs:Date.now()-t0,aiUsed:false,version:'4.0'}};product.grading=r;return r}var base={p1:_computeP1(p),p2:_computeP2(p,ctx.market),p3:_computeP3(p,ctx.portfolio,ctx.isInPortfolio),p4:_computeP4(p,ctx.cat.bestRate)};base.total=Math.round(base.p1*weights.adjustedReturn+base.p2*weights.underlyingQuality+base.p3*weights.portfolioFit+base.p4*weights.riskPremium);base.grade=base.total>=75?'A':base.total>=60?'B':base.total>=45?'C':base.total>=25?'D':'F';var claudeResult=null,final=base,aiUsed=false;try{claudeResult=await _callClaude(ctx,base);if(claudeResult&&claudeResult.adjustments){final=_applyAdjustments(base,claudeResult.adjustments,weights);aiUsed=true}}catch(e){console.warn('[Grader] Claude:',e.message)}function _reason(key,name){var s=name+': base '+base[key];if(aiUsed&&final.deltas[key]!==0){s+=' \u2192 '+(final.deltas[key]>0?'+':'')+final.deltas[key]+' = '+final[key];if(final.reasons[key])s+=' ('+final.reasons[key]+')'}return s}var result={grade:final.grade,score:final.total,killCriteria:{triggered:false,reasons:[]},pillars:{adjustedReturn:{score:final.p1,base:base.p1,delta:final.deltas?final.deltas.p1:0,reasoning:_reason('p1','Rendement')+' | Coupon '+p.coupon+'%'+(p.couponMultiplier>1?' ('+p.couponRaw+'\u00d7'+p.couponMultiplier+')':'')+(p.capitalProtection?', prot\u00e9g\u00e9':', barri\u00e8re '+p.barrier+'%')},underlyingQuality:{score:final.p2,base:base.p2,delta:final.deltas?final.deltas.p2:0,reasoning:_reason('p2','Qualit\u00e9')+(!p.capitalProtection&&p.barrier>0?' [vol/DD 30%]':' [B/Q 35%]')+(ctx.market.worstMetrics?' | Worst: '+ctx.market.worstMetrics.worst_name+' B:'+ctx.market.worstMetrics.worst_buffett:'')},portfolioFit:{score:final.p3,base:base.p3,delta:final.deltas?final.deltas.p3:0,reasoning:ctx.isInPortfolio?'En portefeuille \u2014 neutre':_reason('p3','Fit')},riskPremium:{score:final.p4,base:base.p4,delta:final.deltas?final.deltas.p4:0,reasoning:_reason('p4','Prime')+' | '+p.coupon+'% vs CAT '+ctx.cat.bestRate+'% (spread '+(p.coupon-ctx.cat.bestRate).toFixed(1)+'%)'}},verdict:claudeResult&&claudeResult.verdict?claudeResult.verdict:'Score '+final.total+'/100 (base '+base.total+').',keyRisks:claudeResult&&claudeResult.keyRisks?claudeResult.keyRisks:[],negotiationPoints:claudeResult&&claudeResult.negotiationPoints?claudeResult.negotiationPoints:[],scenarios:claudeResult&&claudeResult.scenarios?claudeResult.scenarios:null,metadata:{gradedAt:new Date().toISOString(),durationMs:Date.now()-t0,aiUsed:aiUsed,version:'4.0',marketDataAvailable:ctx.market.available,catBenchmark:ctx.cat.bestRate,catSource:ctx.cat.source,couponAnnualized:p.coupon,couponRaw:p.couponRaw,couponMultiplier:p.couponMultiplier,couponFrequency:p.couponFrequency,isInPortfolio:ctx.isInPortfolio,hasBarrier:!p.capitalProtection&&p.barrier>0,barrierPct:p.barrier,p2Weights:(!p.capitalProtection&&p.barrier>0)?'vol-dd-30':'buffett-quality-35',baseScore:base.total,baseGrade:base.grade,adjustments:aiUsed?final.deltas:null}};product.grading=result;return result}

async function gradeProposalsBatch(proposals,onProgress){var results=[],total=proposals.length;for(var i=0;i<total;i++){try{var r=await gradeProposal(proposals[i]);results.push({proposal:proposals[i],grading:r})}catch(e){results.push({proposal:proposals[i],grading:{grade:'?',score:null,verdict:'Erreur: '+e.message}})}if(onProgress)onProgress(i+1,total,results[results.length-1]);if(i<total-1)await new Promise(function(r){setTimeout(r,1500)})}results.sort(function(a,b){var o={A:0,B:1,C:2,D:3,F:4,'?':5};return(o[a.grading.grade]||5)-(o[b.grading.grade]||5)});return results}
...
function renderGradingSection(grading){if(!grading)return'<div class="grading-section" style="padding:20px;text-align:center"><button onclick="triggerGrading(this)" class="btn primary" style="padding:12px 24px;border-radius:8px;font-size:14px;cursor:pointer">\ud83c\udfaf Lancer le grading</button></div>';var g=grading,cfg=GRADING_CONFIG.grades[g.grade]||GRADING_CONFIG.grades.F;var isPortfolio=g.metadata&&g.metadata.isInPortfolio;var h='<div class="grading-section"><div style="display:flex;align-items:center;gap:16px;margin-bottom:16px">'+renderGradeBadge(g.grade,g.score,'large')+'<div style="flex:1"><div style="font-size:18px;font-weight:600;color:'+cfg.color+'">Grade '+g.grade+' \u2014 '+cfg.label+'</div><div style="font-size:13px;color:var(--text-muted);margin-top:4px">'+(g.verdict||'')+'</div></div></div>';if(g.killCriteria&&g.killCriteria.triggered)h+='<div style="background:rgba(239,35,60,0.08);border:1px solid rgba(239,35,60,0.25);border-radius:8px;padding:12px;margin-bottom:16px"><div style="font-weight:600;color:#EF233C;margin-bottom:6px">\u26d4 Kill criteria</div>'+g.killCriteria.reasons.map(function(r){return'<div style="font-size:12px;color:#EF233C;padding:2px 0">\u2022 '+r+'</div>'}).join('')+'</div>';if(g.metadata&&g.metadata.couponMultiplier>1)h+='<div style="background:rgba(59,130,246,0.08);border:1px solid rgba(59,130,246,0.2);border-radius:8px;padding:10px;margin-bottom:16px;font-size:12px"><strong style="color:var(--accent)">\u2139 Coupon</strong> : '+g.metadata.couponRaw+'% \u00d7 '+g.metadata.couponMultiplier+' ('+g.metadata.couponFrequency+') = <strong>'+g.metadata.couponAnnualized+'% annuel</strong></div>';if(g.metadata&&g.metadata.aiUsed&&g.metadata.baseScore!==undefined)h+='<div style="font-size:11px;color:var(--text-dim);margin-bottom:12px">Base: '+g.metadata.baseScore+'/100 ('+g.metadata.baseGrade+') \u2192 IA: <strong>'+g.score+'/100 ('+g.grade+')</strong></div>';var pn={adjustedReturn:'Rendement ajust\u00e9',underlyingQuality:'Qualit\u00e9 sous-jacent',riskPremium:'Prime vs CAT'};if(!isPortfolio)pn.portfolioFit='Fit portefeuille';var weights=isPortfolio?GRADING_CONFIG.weightsPortfolio:GRADING_CONFIG.weightsProposal;if(g.pillars){h+='<div style="margin-bottom:16px">';['adjustedReturn','underlyingQuality','portfolioFit','riskPremium'].forEach(function(k){if(!pn[k])return;var pl=g.pillars[k]||{},sc=pl.score;if(sc===null||sc===undefined)return;var w=weights[k];var bc=sc>=70?'#06D6A0':sc>=45?'#FFB627':'#EF233C';var deltaStr=pl.delta&&pl.delta!==0?' <span style="color:'+(pl.delta>0?'#06D6A0':'#EF233C')+';font-size:10px">('+(pl.delta>0?'+':'')+pl.delta+' IA)</span>':'';h+='<div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:3px"><span style="color:var(--text-muted)">'+pn[k]+' ('+Math.round(w*100)+'%)</span><span style="font-weight:600">'+sc+'/100'+deltaStr+'</span></div><div style="height:6px;background:var(--bg-card,var(--surface));border-radius:3px;overflow:hidden"><div style="height:100%;width:'+sc+'%;background:'+bc+';border-radius:3px"></div></div>'+(pl.reasoning?'<div style="font-size:11px;color:var(--text-muted);margin-top:2px">'+pl.reasoning+'</div>':'')+'</div>'});h+='</div>'}if(g.scenarios){h+='<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:16px">';[['optimistic','Optimiste','#06D6A0'],['base','Base','#4ECDC4'],['stress','Stress','#FFB627'],['worst','Worst','#EF233C']].forEach(function(x){var s=g.scenarios[x[0]];if(!s)return;h+='<div style="text-align:center;padding:8px;border-radius:8px;background:'+x[2]+'15;border:1px solid '+x[2]+'30"><div style="font-size:10px;color:var(--text-muted)">'+x[1]+'</div><div style="font-size:16px;font-weight:600;color:'+x[2]+'">'+((s.return_eur||0)>=0?'+':'')+(s.return_eur||0).toLocaleString('fr-FR')+'\u20ac</div><div style="font-size:10px;color:var(--text-muted)">'+((s.return_pct||0)>=0?'+':'')+(s.return_pct||0)+'% \u00b7 '+Math.round((s.probability||0)*100)+'%</div></div>'});h+='</div>'}if(g.keyRisks&&g.keyRisks.length>0)h+='<div style="font-size:12px;color:var(--text-muted);margin-bottom:8px"><strong>Risques :</strong> '+g.keyRisks.join(' \u00b7 ')+'</div>';if(g.grade==='C'&&g.negotiationPoints&&g.negotiationPoints.length>0)h+='<div style="font-size:12px;color:#FFB627;margin-bottom:8px"><strong>\u00c0 n\u00e9gocier :</strong> '+g.negotiationPoints.join(' \u00b7 ')+'</div>';h+='<div style="font-size:10px;color:var(--text-muted);opacity:0.5;margin-top:8px">v4.0'+(g.metadata.aiUsed?' Hybride':' Local')+' \u00b7 '+new Date(g.metadata.gradedAt).toLocaleDateString('fr-FR')+' \u00b7 '+g.metadata.durationMs+'ms \u00b7 CAT '+g.metadata.catBenchmark+'% ('+g.metadata.catSource+')'+(isPortfolio?' \u00b7 Portefeuille':'')+(g.metadata.hasBarrier?' \u00b7 P2 vol/DD 30%':'')+'</div>';h+='</div>';return h;}

window.ProposalGrader={grade:gradeProposal,gradeBatch:gradeProposalsBatch,renderBadge:renderGradeBadge,renderSection:renderGradingSection,checkKillCriteria:_checkKillCriteria,normalize:_graderNormalize,config:GRADING_CONFIG,version:'4.0'};
console.log('[StructBoard] ProposalGrader v4.0 \u2014 post-audit: barrier P1, conditional P2, continuous P4, anti-momentum');
