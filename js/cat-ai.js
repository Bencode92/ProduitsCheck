...
// 1. Optimiseur technique (dans cat.js) — algo, FGDR, ladder
// 2. Analyse IA (ici) — diagnostic, arbitrages, discussion
...
renderCAT = function(container) {
  ...
  // Injecter le bouton IA dans la barre d'actions
  ...
  if (btnBar) {
    ...
    // Insérer avant le bouton "Optimiser"
...
// ─── Contexte portefeuille pour Claude ──────────────────────
...
1. **DIAGNOSTIC** — État global: diversification, rendement, liquidité, risques
2. **CONCENTRATIONS** — Risques par banque (FGDR), par durée, par type
3. **RENDEMENT** — Le taux pondéré est-il bon vs le marché actuel? Quels placements sous-performent?
4. **ARBITRAGES RECOMMANDÉS** — Quoi arrêter, renouveler, augmenter? Quelles banques privilégier?
5. **OPTIMISATION** — Si on ré-allouait tout aujourd'hui, comment structurer?
6. **RISQUES** — Taux (baisse à venir?), liquidité, contrepartie
7. **POINTS D'ATTENTION** — Pénalités, renew auto à surveiller, échéances clés
      ...
      <div class="sheet-main">
          ...
          <div id="cat-ai-analysis-content">
            ${catAIAnalysis
              ...
              : `<div style="text-align:center;padding:30px">
                  <p style="color:var(--text-muted);margin-bottom:16px">Claude va analyser votre portefeuille: concentrations, rendement, risques, arbitrages.</p>
                  <button class="btn ai-glow lg" onclick="launchCATAnalysis()">🚀 Lancer l'analyse</button></div>`}
          </div>
        </div>
        <!-- Chat -->
        <div class="sheet-card" style="min-height:400px;display:flex;flex-direction:column">
          ...
          <div id="cat-chat-messages" style="flex:1;overflow-y:auto;max-height:400px;margin-bottom:12px">
            <div class="chat-msg system"><div class="chat-msg-content">💡 Posez vos questions. Claude a accès à tous vos placements, taux, objectifs et à l'analyse.</div></div>
            ${catAIConversation.map(m => `
              <div class="chat-msg ${m.role}"><div class="chat-msg-avatar">${m.role==='user'?'👤':'🤖'}</div>
              <div class="chat-msg-content">${m.role==='assistant'?formatAIText(m.content):escapeHTML(m.content)}</div></div>`).join('')}
          </div>
          <div class="chat-input-area">
            <textarea id="cat-chat-input" class="chat-input" placeholder="Ex: Dois-je renouveler mon CAT SG? Comment réduire ma concentration?"
      ...
      <div class="sheet-sidebar">
        ...
        <div class="sheet-card"><h3 class="sheet-card-title">Questions rapides</h3>
          <div class="action-buttons">
            ...
            <button class="btn" style="width:100%" onclick="askCATQuestion('Si les taux baissent de 1%, quel impact?')">Impact taux?</button>
          </div>
        </div>
      </div>
    </div>`;
...
function renderCATSidebarStats() {
  ...
  return `<div style="font-size:12px;line-height:1.8">
    ...
    ${stats.fgdrAlerts.length > 0 ? `<div style="color:var(--red);margin-top:8px">⚠️ FGDR: ${stats.fgdrAlerts.length} alerte(s)</div>` : ''}</div>`;
}

async function launchCATAnalysis() {
  const content = document.getElementById('cat-ai-analysis-content');
  content.innerHTML = '<div style="display:flex;align-items:center;gap:10px;padding:20px;color:var(--text-muted)"><div class="spinner"></div>Analyse en cours...</div>';
  try {
  ...
  } catch (e) { content.innerHTML = `<div style="color:var(--red);padding:20px">Erreur: ${e.message}</div>`; }
}
