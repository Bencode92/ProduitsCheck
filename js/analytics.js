// ═══════════════════════════════════════════════════════════════
// STRUCTBOARD — Analytique & Simulations
// Graphiques, rendements, flux de trésorerie, répartition
// ═══════════════════════════════════════════════════════════════
...
function projectCashflows(years) {
  ...
  for (let y = 0; y < years; y++) {
    ...
    let structured = 0, cat = 0, total = 0;

    // Produits structurés: coupon annuel
    products.forEach(p => {
      const maturityYear = p.maturityDate ? new Date(p.maturityDate).getFullYear() : (now.getFullYear() + 20);
      if (year <= maturityYear) {
        structured += calcProductAnnualYield(p);
      }
    });

    // CAT: intérêts
    catDeposits.forEach(d => {
      const matDate = d.maturityDate ? new Date(d.maturityDate) : null;
      const matYear = matDate ? matDate.getFullYear() : (now.getFullYear() + 5);
      if (year <= matYear) {
        const amount = parseFloat(d.amount) || 0;
        const rate = parseFloat(d.rate) || 0;
        cat += Math.round(amount * rate / 100);
      }
    });

    total = structured + cat;
    flows.push({ year, structured, cat, total });
  }
  return flows;
}
...
function getDistributionByBank() {
  ...
  const map = {};
  products.forEach(p => {
    const bank = BANKS.find(b => b.id === p.bankId)?.name || p.bankId || 'Non assigné';
    map[bank] = (map[bank] || 0) + (parseFloat(p.investedAmount) || 0);
  });
  catDeposits.forEach(d => {
    const bank = d.bankName || d.bankId || 'Non assigné';
    map[bank] = (map[bank] || 0) + (parseFloat(d.amount) || 0);
  });
...
function getDistributionByEntity() {
  ...
  const map = {};
  products.forEach(p => {
    const entity = p.entity ? (MY_ENTITIES.find(e => e.id === p.entity)?.name || p.entity) : 'Non assigné';
    map[entity] = (map[entity] || 0) + (parseFloat(p.investedAmount) || 0);
  });
  return Object.entries(map).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value);
}
...
function getDistributionByType() {
  ...
  const map = {};
  products.forEach(p => {
    const type = PRODUCT_TYPES.find(t => t.id === p.type)?.name || p.type || 'Autre';
    map[type] = (map[type] || 0) + (parseFloat(p.investedAmount) || 0);
  });
  if (catDeposits.length > 0) {
    const catTotal = catDeposits.reduce((s, d) => s + (parseFloat(d.amount) || 0), 0);
    map['CAT + Parts Sociales'] = (map['CAT + Parts Sociales'] || 0) + catTotal;
  }
  return Object.entries(map).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value);
}
...
function getMaturityProfile() {
  ...
  products.forEach(p => {
    if (p.maturityDate) {
      const y = new Date(p.maturityDate).getFullYear();
      if (!map[y]) map[y] = { structured: 0, cat: 0 };
      map[y].structured += parseFloat(p.investedAmount) || 0;
    }
  });
  catDeposits.forEach(d => {
    if (d.maturityDate) {
      ...
      if (!map[y]) map[y] = { structured: 0, cat: 0 };
      map[y].cat += parseFloat(d.amount) || 0;
    }
  });
...
function renderAnalytics(container) {
  ...
  container.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px">
      <!-- Rendement annuel par produit -->
      <div class="fiche-section"><div class="fiche-section-header"><span class="fiche-section-icon">📊</span><span class="fiche-section-title">Rendement Annuel par Produit</span></div>
        <div class="fiche-section-body"><canvas id="chart-yield" height="280"></canvas></div></div>

      <!-- Projection flux -->
      <div class="fiche-section"><div class="fiche-section-header"><span class="fiche-section-icon">📈</span><span class="fiche-section-title">Projection Flux de Trésorerie (10 ans)</span></div>
        <div class="fiche-section-body"><canvas id="chart-cashflow" height="280"></canvas></div></div>

      <!-- Répartition par banque -->
      <div class="fiche-section"><div class="fiche-section-header"><span class="fiche-section-icon">🏦</span><span class="fiche-section-title">Répartition par Banque</span></div>
        <div class="fiche-section-body"><canvas id="chart-bank" height="280"></canvas></div></div>

      <!-- Répartition par type -->
      <div class="fiche-section"><div class="fiche-section-header"><span class="fiche-section-icon">🎯</span><span class="fiche-section-title">Répartition par Type</span></div>
        <div class="fiche-section-body"><canvas id="chart-type" height="280"></canvas></div></div>

      <!-- Répartition par entreprise -->
      <div class="fiche-section"><div class="fiche-section-header"><span class="fiche-section-icon">🏢</span><span class="fiche-section-title">Répartition par Entreprise</span></div>
        <div class="fiche-section-body"><canvas id="chart-entity" height="280"></canvas></div></div>

      <!-- Profil de maturité -->
      <div class="fiche-section"><div class="fiche-section-header"><span class="fiche-section-icon">⏳</span><span class="fiche-section-title">Profil de Maturité</span></div>
        <div class="fiche-section-body"><canvas id="chart-maturity" height="280"></canvas></div></div>
    </div>

    <!-- Tableau détaillé rendement -->
      ...
      <div class="fiche-section-body">
        <table style="width:100%;font-size:12px;border-collapse:collapse">
          <thead><tr style="border-bottom:2px solid var(--border);text-align:left">
            ...
            <th style="padding:8px 6px;color:var(--text-dim);font-size:10px;text-transform:uppercase">Type</th>
            ...
            <th style="padding:8px 6px;color:var(--text-dim);font-size:10px;text-transform:uppercase;text-align:right">Coupon</th>
          ...
          <tbody>
            ${products.map(p => {
              ...
              return `<tr style="border-bottom:1px solid var(--border)">
                ...
                <td style="padding:8px 6px;color:var(--text-muted)">${PRODUCT_TYPES.find(t => t.id === p.type)?.name || '—'}</td>
                ...
                <td style="padding:8px 6px;text-align:right;color:var(--green);font-family:var(--mono)">${p.coupon?.rate ? formatPct(p.coupon.rate) : '—'}</td>
                <td style="padding:8px 6px;text-align:right;color:var(--green);font-family:var(--mono);font-weight:600">${formatNumber(calcProductAnnualYield(p))}€</td>
            ...
            ${catDeposits.map(d => {
              ...
              return `<tr style="border-bottom:1px solid var(--border)">
                ...
                <td style="padding:8px 6px;color:var(--text-muted)">${d.productType === 'parts-sociales' ? 'Parts Sociales' : 'CAT'}</td>
                <td style="padding:8px 6px">—</td>
                <td style="padding:8px 6px;text-align:right;font-family:var(--mono)">${formatNumber(d.amount)}€</td>
                <td style="padding:8px 6px;text-align:right;color:var(--green);font-family:var(--mono)">${d.rate ? formatPct(d.rate) : '—'}</td>
                <td style="padding:8px 6px;text-align:right;color:var(--green);font-family:var(--mono);font-weight:600">${formatNumber(interest)}€</td>
          ...
          <tfoot><tr style="border-top:2px solid var(--border);font-weight:700">
            <td style="padding:10px 6px;color:var(--text-bright)" colspan="3">TOTAL</td>
...
function renderYieldChart() {
  ...
  const { products, catDeposits } = getPortfolioData();
  const items = [
    ...products.map(p => ({ name: (p.name || 'Produit').substring(0, 20), yield: calcProductAnnualYield(p), color: '#3B82F6' })),
    ...catDeposits.map(d => ({ name: (d.productName || 'CAT').substring(0, 20), yield: Math.round((parseFloat(d.amount)||0)*(parseFloat(d.rate)||0)/100), color: '#06D6A0' })),
  ].filter(i => i.yield > 0);

  new Chart(ctx, {
    type: 'bar',
    data: { labels: items.map(i => i.name), datasets: [{ label: 'Rendement annuel (€)', data: items.map(i => i.yield), backgroundColor: items.map((_, i) => CHART_COLORS[i % CHART_COLORS.length] + 'CC'), borderRadius: 6, borderSkipped: false }] },
...
function renderCashflowChart() {
  ...
  new Chart(ctx, {
    type: 'bar',
    data: {
      labels: flows.map(f => f.year.toString()),
      datasets: [
    ...
    options: {
      responsive: true,
      plugins: { tooltip: { callbacks: { label: (c) => c.dataset.label + ': ' + formatNumber(c.raw) + '€' } } },
      scales: {
        y: { stacked: true, grid: { color: chartDefaults.borderColor }, ticks: { callback: v => formatNumber(v) + '€' } },
        y1: { position: 'right', grid: { display: false }, ticks: { callback: v => formatNumber(v) + '€' } },
...
function renderBankPieChart() {
  const ctx = document.getElementById('chart-bank');
  if (!ctx) return;
  const data = getDistributionByBank();
  new Chart(ctx, {
    type: 'doughnut',
    data: { labels: data.map(d => d.name), datasets: [{ data: data.map(d => d.value), backgroundColor: data.map((_, i) => CHART_COLORS[i % CHART_COLORS.length]), borderWidth: 0 }] },
...
function renderTypePieChart() {
  ...
  if (!ctx) return;
  const data = getDistributionByType();
  new Chart(ctx, {
    type: 'doughnut',
    data: { labels: data.map(d => d.name), datasets: [{ data: data.map(d => d.value), backgroundColor: data.map((_, i) => CHART_COLORS[(i + 3) % CHART_COLORS.length]), borderWidth: 0 }] },
    options: { responsive: true, cutout: '55%', plugins: { tooltip: { callbacks: { label: (c) => c.label + ': ' + formatNumber(c.raw) + '€' } }, legend: { position: 'right', labels: { boxWidth: 12, padding: 8 } } } },
  });
}

function renderEntityPieChart() {
  const ctx = document.getElementById('chart-entity');
  if (!ctx) return;
  const data = getDistributionByEntity();
  const entityColors = data.map(d => {
    const ent = MY_ENTITIES.find(e => e.name === d.name);
    return ent?.color || '#64748B';
  });
  new Chart(ctx, {
    type: 'doughnut',
    data: { labels: data.map(d => d.name), datasets: [{ data: data.map(d => d.value), backgroundColor: entityColors, borderWidth: 0 }] },
    options: { responsive: true, cutout: '55%', plugins: { tooltip: { callbacks: { label: (c) => c.label + ': ' + formatNumber(c.raw) + '€' } }, legend: { position: 'right', labels: { boxWidth: 12, padding: 8 } } } },
  });
}

function renderMaturityChart() {
  const ctx = document.getElementById('chart-maturity');
  if (!ctx) return;
  const data = getMaturityProfile();
  new Chart(ctx, {
    type: 'bar',
    data: {
    ...
    options: { responsive: true, plugins: { tooltip: { callbacks: { label: (c) => c.dataset.label + ': ' + formatNumber(c.raw) + '€' } } }, scales: { y: { stacked: true, grid: { color: chartDefaults.borderColor }, ticks: { callback: v => formatNumber(v) + '€' } }, x: { stacked: true, grid: { display: false } } } },
  });
}
