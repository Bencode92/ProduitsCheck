// ═══ CAT PATCHES V2i — Separate confirmed vs scanned rates with labels ═══
...
async function deleteMarketRate(idx) { if (!confirm('Supprimer ce taux ?')) return; catManager.rates.rates.splice(idx, 1); catManager.rates.lastUpdated = new Date().toISOString(); await catManager.saveRates(); showToast('Supprimé', 'success'); renderCAT(document.getElementById('main-content')); }
function _isRateExpired(r) { if (!r.date) return false; const d = new Date(r.date); if (isNaN(d.getTime())) return false; return (new Date() - d) / 86400000 > 5; }

// ═══ RENDER RATE CARD (shared between confirmed + scanned) ═══
function _renderRateCard(r, isScanned) {
  ...
  let scheduleText = '';
  if (isProgressive && r.rateSchedule && Array.isArray(r.rateSchedule)) { try { scheduleText = r.rateSchedule.map(s => s.rate + '%').join(' → '); } catch(e) {} }

  const confBadge = r.confidence
    ? `<span style="font-size:8px;color:${r.confidence === 'high' ? 'var(--green)' : r.confidence === 'medium' ? 'var(--orange)' : 'var(--red)'}; margin-left:4px">${r.confidence === 'high' ? '✅' : r.confidence === 'medium' ? '🟡' : '🟠'}</span>`
    : '';

  // Source label
  const sourceLabel = isScanned
    ? '<div style="font-size:8px;color:var(--purple);margin-top:3px;display:flex;align-items:center;gap:3px"><span style="background:var(--purple);color:white;padding:1px 5px;border-radius:8px;font-weight:600;font-size:7px;text-transform:uppercase">Web</span> 📞 À contacter</div>'
    : '';

  return `<div style="background:var(--bg-elevated);border:1px solid ${expired ? 'var(--orange)' : isScanned ? 'rgba(139,92,246,0.3)' : 'var(--border)'};border-radius:var(--radius-sm);padding:10px 12px;position:relative;${expired ? 'opacity:0.5;' : ''}${isScanned ? 'border-left:3px solid var(--purple);' : ''}">
    <button onclick="event.stopPropagation();deleteMarketRate(${r._idx})" style="position:absolute;top:4px;right:4px;background:none;border:none;color:var(--text-dim);cursor:pointer;font-size:14px;padding:2px 6px;border-radius:4px;line-height:1" title="Supprimer" onmouseover="this.style.color='var(--red)';this.style.background='rgba(239,68,68,0.1)'" onmouseout="this.style.color='var(--text-dim)';this.style.background='none'">✕</button>
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;padding-right:20px">
      <span style="font-size:12px;font-weight:600;color:var(--text-bright)">${name}${confBadge}</span>
      <span style="font-family:var(--mono);color:var(--green);font-size:14px;font-weight:700">${r.rate}%</span>
    </div>
    <div style="font-size:10px;color:var(--text-muted)">${r.durationMonths} mois · ${isProgressive ? '📈 progressif' : '📊 fixe'}${expired ? ' · <span style="color:var(--orange)">⏰</span>' : ''}</div>
...
// ═══ 7. renderCAT — MARKET RATES with 2 sections ═══════
  ...
  try {
    ...
    if (rates.length === 0) {
      ratesHTML += `<div style="text-align:center;padding:30px;color:var(--text-dim)">
        ...
        <div style="font-size:12px;margin-bottom:12px">Aucun taux importé</div>
        <div style="display:flex;gap:8px;justify-content:center">
          <button class="btn ai-glow" onclick="showBankScannerModal()">🔍 Scanner les banques</button>
          <button class="btn" onclick="showCATRatesModal()">✏️ Import manuel</button>
        </div>
      </div>`;
    }

    // ═══ SECTION 1: Confirmed rates (from email/manual import) ═══
    if (confirmedRates.length > 0) {
      const confirmedByBank = groupByBank(confirmedRates);
      Object.entries(confirmedByBank).forEach(([bankId, group]) => {
        ...
        ratesHTML += `<div style="margin-bottom:16px">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
            ...
            <span style="font-size:11px;color:var(--text-dim)">${group.rates.length} produit${group.rates.length > 1 ? 's' : ''}${group.rates[0]?.date ? ' · Valide: ' + group.rates[0].date : ''}</span>
          </div>
          <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:8px">`;
        sorted.forEach(r => { ratesHTML += _renderRateCard(r, false); });
        ratesHTML += `</div></div>`;
      });
    }

    // ═══ SECTION 2: Scanned rates (web scan — indicatifs) ═══
    if (scannedRates.length > 0) {
      ratesHTML += `<div style="margin-top:${confirmedRates.length > 0 ? '24px' : '0'};padding-top:${confirmedRates.length > 0 ? '16px' : '0'};${confirmedRates.length > 0 ? 'border-top:1px dashed var(--border);' : ''}">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
          <div style="display:flex;align-items:center;gap:8px">
            <span style="font-size:14px">📞</span>
            <strong style="font-size:13px;color:var(--purple)">Taux indicatifs — à contacter</strong>
            <span style="font-size:10px;color:var(--text-dim);background:var(--bg-elevated);padding:2px 8px;border-radius:10px">${activeScanned.length} taux · source web</span>
          </div>
          <button class="btn sm ai-glow" onclick="showBankScannerModal()" style="font-size:10px">🔄 Mettre à jour</button>
        </div>
        <div style="font-size:10px;color:var(--text-dim);margin-bottom:12px;padding:8px;background:rgba(139,92,246,0.05);border:1px solid rgba(139,92,246,0.15);border-radius:var(--radius-sm)">
          Ces taux sont indicatifs (source web, grand public). Contactez les banques pour obtenir les conditions entreprise négociées.
        </div>`;

      const scannedByBank = groupByBank(scannedRates);
      Object.entries(scannedByBank).forEach(([bankId, group]) => {
        const sorted = [...group.rates].sort((a, b) => a.durationMonths - b.durationMonths);
        ratesHTML += `<div style="margin-bottom:16px">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
            <span class="bank-dot" style="background:var(--purple)"></span>
            <strong style="font-size:12px;color:var(--text-bright)">${group.name}</strong>
            <span style="font-size:10px;color:var(--text-dim)">${group.rates.length} taux</span>
            ${group.rates[0]?.scanNote ? '<span style="font-size:9px;color:var(--orange)">ℹ️ ' + group.rates[0].scanNote + '</span>' : ''}
          </div>
          <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:8px">`;
        sorted.forEach(r => { ratesHTML += _renderRateCard(r, true); });
        ratesHTML += `</div></div>`;
      });
      ratesHTML += `</div>`;
    }
