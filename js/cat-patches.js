// ═══ CAT PATCHES V2g — Delete rate + expired rates auto-grayed ═══
...
// ═══ DELETE MARKET RATE ══════════════════════════════════
async function deleteMarketRate(idx) {
  if (!confirm('Supprimer ce taux ?')) return;
  catManager.rates.rates.splice(idx, 1);
...
function _isRateExpired(r) {
  if (!r.date) return false;
  // Date is the validity date — check if it's a "end of month" date
  const d = new Date(r.date);
  if (isNaN(d.getTime())) return false;
  const now = new Date();
  // If the date is in the past, rate is expired
  // Add buffer: consider expired if date is more than 5 days old
  ...
  try {
    ...
    if (rates.length > 0) {
      ...
      rates.forEach((r, idx) => {
        ...
        byBank[key].rates.push({ ...r, _idx: idx });
      });

      // Count active vs expired
      const activeRates = rates.filter(r => !_isRateExpired(r));
      const expiredRates = rates.filter(r => _isRateExpired(r));
      ...
      let ratesHTML = `<div class="section">
        <div class="section-header">
          ...
          <div style="display:flex;gap:8px;align-items:center">
            <span style="font-size:11px;color:var(--text-dim)">${activeRates.length} actif${activeRates.length > 1 ? 's' : ''}${expiredRates.length > 0 ? ' · <span style=color:var(--orange)>' + expiredRates.length + ' expiré' + (expiredRates.length > 1 ? 's' : '') + '</span>' : ''}</span>
      ...
      Object.entries(byBank).forEach(([bankId, group]) => {
        ...
        sorted.forEach(r => {
          ...
          ratesHTML += `<div style="background:var(--bg-elevated);border:1px solid ${expired ? 'var(--orange)' : 'var(--border)'};border-radius:var(--radius-sm);padding:10px 12px;position:relative;${expired ? 'opacity:0.5;' : ''}">
            ...
            <div style="font-size:10px;color:var(--text-muted)">${r.durationMonths} mois · ${isProgressive ? '📈 progressif' : '📊 fixe'}${expired ? ' · <span style="color:var(--orange)">⏰ Expiré</span>' : ''}</div>
