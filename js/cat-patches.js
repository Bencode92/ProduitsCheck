...
function _isRateExpired(r) { if (!r.date) return false; const d = new Date(r.date); if (isNaN(d.getTime())) return false; const ageDays = (new Date() - d) / 86400000; const maxAge = r.source === 'web scan' ? 5 : 45; /* seul le scan web est indicatif (5j) ; saisie manuelle/confirmée/legacy sans source = 45j */ return ageDays > maxAge; }
