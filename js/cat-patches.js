...
function showBrochureContractsModal() {
  ...
  const hasSchedule = p.rateSchedule && p.rateSchedule.length > 0;
  ...
  modal.innerHTML = `<div class="modal-overlay" onclick="closeModal()"><div class="modal-content modal-large" onclick="event.stopPropagation()">
    ...
    <div id="bc-contracts">
      <div class="bc-contract-row" style="display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:8px;margin-bottom:8px;align-items:end">
        ...
        <div class="form-field" style="margin:0"><label style="font-size:10px">Montant (€)</label><input class="bc-amount" type="number" value="${p.amount || ''}" placeholder="50000"></div>
        <div class="form-field" style="margin:0"><label style="font-size:10px">Date souscription</label><input class="bc-date" type="date" value="${p.startDate || new Date().toISOString().split('T')[0]}"></div>
...
async function handleSaveBrochureContracts() {
  ...
  const bankId=document.getElementById('bc-bank')?.value; const bank=BANKS.find(bk=>bk.id===bankId);
  ...
  const dur=parseInt(document.getElementById('bc-duration')?.value)||0;
  ...
  for(const c of contracts){
    catManager.addDeposit({
      ...
      productName: b.parsed?.productName||'CAT '+(dur||'')+'m',
      amount:c.amount, rate, rateType: aiSchedule.length > 0 ? 'progressif' : (b.parsed?.rateType||'fixe'),
      rateSchedule: aiSchedule.length > 0 ? aiSchedule : undefined,
      durationMonths:dur, startDate:c.startDate,
...
showEditPlacementModal = function(id) {
  ...
  _injectRateScheduleSection(d, null);
