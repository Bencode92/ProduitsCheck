// ═══════════════════════════════════════════════════════════════
// STRUCTBOARD — Grader P1 Expected Return Patch v1.2
//
// v1.2: Basket vol = average (not max) + isBasket flag
...
(function() {
  ...
  function _probAbove(trigger, volPct, T, rPct) {
    var sigma = volPct / 100; var r = rPct / 100; T = Math.max(0.25, T);
  ...
  function _probBreach(barrierPct, volPct, T, rPct) {
    ...
    var sigma = volPct / 100; var r = rPct / 100; T = Math.max(0.25, T);
    var mu = r - sigma * sigma / 2; var sqrtT = Math.sqrt(T);
  ...
  function _loadVolData() {
    ...
    return Promise.all([p1, p2, p3]).then(function() {
      ...
      console.log('[p1-bs] Vol data loaded: ' + Object.keys(_volData.stocks).length + ' stocks, ' + Object.keys(_volData.indices).length + ' indices');
      return _volData;
    });
  }
  ...
  function _resolveVol(name) {
    ...
    for (var t = 0; t < tokens.length; t++) {
      ...
      if (_volData.indices[tok]) return _volData.indices[tok].realVol || _volData.indices[tok].defaultVol;
    }
    var keys = Object.keys(_volData.indices);
    for (var i = 0; i < keys.length; i++) {
      if (keys[i].indexOf(n) >= 0 || n.indexOf(keys[i]) >= 0) {
        return _volData.indices[keys[i]].realVol || _volData.indices[keys[i]].defaultVol;
      }
    }
  ...
  // v1.2: detect basket from underlyingType, structureType, or mechanism keywords
  function _isBasketProduct(product) {
    var ut = (product.underlyingType || '').toLowerCase();
    var st = (product.structureType || '').toLowerCase();
    if (ut === 'basket') return true;
    if (st === 'basket') return true;
    var mech = (product.mechanism || '').toLowerCase();
    var name = (product.name || '').toLowerCase();
    if (/panier\s*[eé]qui/i.test(mech) || /panier\s*[eé]qui/i.test(name)) return true;
    if (/niveau\s*du\s*panier/i.test(mech)) return true;
    if (/basket|equi.?pond/i.test(mech)) return true;
    return false;
  }
  ...
  function _extractBSInputs(product) {
    ...
    var annRate = (typeof window.getAnnualizedRate === 'function')
      ? window.getAnnualizedRate(product) : (parseFloat(coupon.rate || coupon) || 0);
    ...
    if ((!unds || unds.length === 0) && product.aiParsed) unds = product.aiParsed.underlyings || [];
    ...
    // v1.2: basket detection
    var isBasket = _isBasketProduct(product);
    // If basket detected, override worst-of
    ...
    return {
      ...
      isBasket: isBasket, hasMemory: hasMemory, dispMedianReturn: dispMedianReturn
    };
  }

  function _resolveVols(inputs) {
    var vols = []; var defaultVol = DEFAULT_VOLS[inputs.undType] || 25;
    if (inputs.underlyings.length > 0) {
      inputs.underlyings.forEach(function(u) { var v = _resolveVol(u); vols.push(v || defaultVol); });
    } else { vols.push(defaultVol); }
    return vols;
  }

  // v1.2: compute average vol for basket (with correlation adjustment)
  function _basketVol(vols, corr) {
    corr = corr || 0.4; // typical intra-basket correlation
    var n = vols.length;
  ...
  function _computeBSP1(inputs, riskFreeRate) {
    ...
    if (inputs.isDispersion) {
      probCoupon = 0.95; couponEffectif = inputs.dispMedianReturn;
      log.push('Dispersion: prob=95%, rdt median hist=' + couponEffectif + '%');
    } else if (inputs.isRate) {
      probCoupon = 0.95; couponEffectif = inputs.annRate;
      log.push('Taux fixe: prob=95%, coupon=' + couponEffectif + '%');
    // v1.2: BASKET branch — use average vol of the portfolio
    } else if (inputs.isBasket && vols.length > 1) {
      var bVol = _basketVol(vols, 0.4);
      probCoupon = _probAbove(inputs.triggerCoupon, bVol, inputs.matEsperee, r);
      couponEffectif = inputs.annRate;
      log.push('Basket vol=' + bVol.toFixed(0) + '% (avg ' + vols.length + ' SJ corr=0.4) | P(panier>' +
        inputs.triggerCoupon + '% T=' + inputs.matEsperee.toFixed(1) + 'a) = ' + (probCoupon * 100).toFixed(1) + '%');
    } else if (inputs.isWorstOf && vols.length > 1) {
      var probs = vols.map(function(v) { return _probAbove(inputs.triggerCoupon, v, inputs.matEsperee, r); });
    ...
    if (!inputs.isProtected && inputs.barrierCapital > 0 && inputs.barrierCapital < 100) {
      // v1.2: for baskets, use basket vol for breach prob too
      var volForBreach;
      if (inputs.isBasket && vols.length > 1) {
        volForBreach = _basketVol(vols, 0.4);
      } else {
        volForBreach = vols.length > 0 ? Math.max.apply(null, vols) : 25;
      }
      var probB = _probBreach(inputs.barrierCapital, volForBreach, inputs.matEsperee, r);
    ...
    if (inputs.drag > 0) log.push('Drag decrement: -' + inputs.drag.toFixed(2) + '%/an');
    log.push('Rdt net: ' + rendementEspere.toFixed(1) + '% - ' + perteEsperee.toFixed(1) + '%' +
      (inputs.drag > 0 ? ' - ' + inputs.drag.toFixed(1) + '%' : '') + ' = ' + rendementNet.toFixed(1) + '%/an');
    ...
    return {
      ...
      reasoning: log.join(' | '), method: 'BS-simplifie-v1.2'
    };
  }
  ...
  function _patchGrade() {
    ...
    ProposalGrader.grade = function(product) {
      return _loadVolData().then(function() {
        ...
        if (result.metadata) {
          ...
          result.metadata.isBasket = inputs.isBasket;
        }
    ...
    console.log('[p1-bs v1.2] Black-Scholes P1 (basket vol avg) active');
    return true;
  }

  if (!_patchGrade()) {
    var attempts = 0;
    var iv = setInterval(function() { attempts++; if (_patchGrade() || attempts > 60) clearInterval(iv); }, 100);
  }
})();
