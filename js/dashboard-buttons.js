...
function _renderPortfolioSummaryTable(state) {
    ...
    portfolio.forEach(function(p) {
        ...
        if (grade === '-') { statusHtml = '<span style="color:#94A3B8;font-weight:600">$ CASH</span>'; statusType = 'cash'; }
            ...
            var _gtag = tracking.guaranteedActive ? '<span style="font-size:8px;opacity:0.85;font-weight:500"> \u00b7 garanti jusqu\u2019au ' + (tracking.guaranteedUntil ? tracking.guaranteedUntil.toLocaleDateString('fr-FR') : '') + (tracking.couponOKByRate === false ? ' \u00b7 \u26a0 taux au-dessus de la barri\u00e8re ensuite' : '') + '</span>' : (tracking.guaranteedYears ? '<span style="font-size:8px;opacity:0.75;font-weight:500"> \u00b7 ' + tracking.guaranteedYears + 'a garantis</span>' : '');
