// ═══════════════════════════════════════════════════════════════
// STRUCTBOARD — Smart CAT Optimizer V5b — Web rates transparency
// ═══════════════════════════════════════════════════════════════
...
function buildOptimizationAnalysis() {
  ...
  // Track which web rates were used as best alternative
  ...
  const depositAnalysis=active.map(d=>{
    ...
    const compatible=Object.values(bestByDuration).filter(r=>{if(r.rate<=rate)return false;if(remainingMonths>6&&Math.abs(r.durationMonths-remainingMonths)>6)return false;return true;}).sort((a,b)=>b.rate-a.rate);
    ...
    const normNG=Math.min(1,Math.max(0,netGainAnnual/Math.max(amount*0.01,1)));
    ...
    if(bestAlt&&netGainAnnual>0&&adjustedSpread>=dynThreshold){recommendation='ARBITRER';reason=`NET +${formatNumber(netGainAnnual)}€/an → ${bestAlt.productName||bestAlt.bankName+' '+bestAlt.durationMonths+'m'}`;}
    else if(bestAlt&&spread>0){recommendation='SURVEILLER';reason=netGainAnnual<=0?`Brut +${formatNumber(grossGainAnnual)}€ mais NET ${formatNumber(netGainAnnual)}€ (coûts -${formatNumber(totalOneTimeCosts)}€)`:`Écart ${spread.toFixed(2)}% < seuil ${dynThreshold.toFixed(2)}%`;}
    else if(rate>=(bestOverall?.rate||0)){reason='Leader marché';}
    else{reason='Taux compétitif';}

    return{id:d.id,name:d.productName||'CAT',bankName:d.bankName,entity:d.entityName||'',amount,rate,currentPeriodRate,durationMonths,elapsedMonths,remainingMonths,interestPerYear,
      ...
      grossGainAnnual,exitPenaltyCost,noticeOpportunityCost,netGainAnnual,netGainRemaining:Math.round(netGainAnnual*yearsRemaining*100)/100,
      switchGainPerYear:netGainAnnual,dynamicThreshold:dynThreshold,fgdrScore:fgdrSc,fgdrBonus,compositeScore,
  ...
  const totalNetGain=arbs.reduce((s,d)=>s+d.netGainAnnual,0);
  ...
  // Build web rates summary for transparency
...
showCATSimulator = function() {
  ...
  modal.innerHTML=`<div class="modal-overlay" onclick="closeModal()"><div class="modal-content modal-large" onclick="event.stopPropagation()" style="max-height:90vh;overflow-y:auto">
    ...
    <div style="background:var(--bg-elevated);border:1px solid var(--border);border-radius:var(--radius);padding:16px;margin-bottom:16px">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">
        ...
        <label style="flex:1;min-width:200px;display:flex;align-items:center;gap:10px;padding:12px;background:var(--bg-card);border:2px solid var(--border);border-radius:var(--radius-sm);cursor:pointer" id="opt-src-all" onclick="document.getElementById('opt-radio-all').checked=true;this.style.borderColor='var(--purple)';document.getElementById('opt-src-confirmed').style.borderColor='var(--border)'">
          ...
          <div><div style="font-size:12px;font-weight:600;color:var(--purple)">🔍 Tous (+ web)</div><div style="font-size:10px;color:var(--text-dim)">${allRates.length} taux (${cR.length} confirmés + ${sR.length} web)</div></div>
        </label>
      </div>
    </div>
    <div style="background:rgba(59,130,246,0.05);border:1px solid rgba(59,130,246,0.15);border-radius:var(--radius-sm);padding:10px;margin-bottom:16px;font-size:10px;color:var(--text-muted)">
      <strong style="color:var(--accent)">v2</strong> : seuil dynamique, gain NET après earlyRate + préavis 32j, score FGDR.</div>
...
function launchOptimizer() {
  ...
  const r=document.getElementById('optimizer-results');
  r.innerHTML=`<div style="display:flex;align-items:center;gap:10px;padding:20px;color:var(--text-muted)"><div class="spinner"></div>Analyse v2...</div>`;
...
async function runSmartOptimizer() {
  ...
  try{
    ...
    const srcBadge=_optimizerRateSource==='confirmed'
      ?'<span style="display:inline-block;padding:3px 10px;border-radius:10px;font-size:10px;font-weight:600;color:var(--green);background:rgba(6,214,160,0.1);border:1px solid rgba(6,214,160,0.3);margin-bottom:12px">✅ vs '+analysis._confirmedCount+' taux confirmés · algo v2</span>'
      :'<span style="display:inline-block;padding:3px 10px;border-radius:10px;font-size:10px;font-weight:600;color:var(--purple);background:rgba(139,92,246,0.1);border:1px solid rgba(139,92,246,0.3);margin-bottom:12px">🔍 vs '+analysis._rateCount+' taux ('+analysis._confirmedCount+' confirmés + '+analysis._webCount+' web) · algo v2</span>';
    ...
    showToast('Optimisation v2 OK','success');
...
function renderOptimizationTable(analysis) {
  ...
  if(totalExitCost>0||totalNoticeCost>0){
    html+=`<div style="display:flex;gap:16px;margin-bottom:12px;padding:8px 12px;background:rgba(245,158,11,0.05);border:1px solid rgba(245,158,11,0.15);border-radius:var(--radius-sm);font-size:10px"><span style="color:var(--text-muted)">Coûts :</span><span style="color:var(--orange)">earlyRate: <strong>-${formatNumber(totalExitCost)}€</strong></span><span style="color:var(--orange)">Préavis: <strong>-${formatNumber(totalNoticeCost)}€</strong></span><span style="color:var(--green)">Brut: <strong>+${formatNumber(totalGrossGain)}€/an</strong></span></div>`;
  }

  // ═══ WEB RATES TRANSPARENCY BOX — only when source=all ═══
  if(_optimizerRateSource==='all'&&_webRatesSummary&&_webRatesSummary.length>0){
    const usedWeb=_webRatesSummary.filter(r=>r.usedAsAlt);
    const unusedWeb=_webRatesSummary.filter(r=>!r.usedAsAlt);
    const bestConfirmedRate=Math.max(...(catManager.rates?.rates||[]).filter(r=>r.source!=='web scan'&&!_isRateExpired(r)).map(r=>r.rate),0);
    const bestWebRate=Math.max(..._webRatesSummary.map(r=>r.rate),0);

    html+=`<div style="margin-bottom:12px;padding:12px;background:rgba(139,92,246,0.05);border:1px solid rgba(139,92,246,0.2);border-radius:var(--radius-sm)">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
        <span style="font-size:12px;font-weight:600;color:var(--purple)">📞 ${_webRatesSummary.length} taux web évalués</span>
        <span style="font-size:10px;color:var(--text-dim)">${_webRatesUsedCount} utilisé${_webRatesUsedCount>1?'s':''} comme alternative</span>
      </div>`;

    if(usedWeb.length>0){
      html+=`<div style="margin-bottom:6px;font-size:10px;color:var(--green)">✅ <strong>${usedWeb.length} taux web retenus</strong> comme meilleure alternative :</div>`;
      usedWeb.forEach(r=>{html+=`<div style="font-size:10px;padding:2px 0;color:var(--text-bright)">  → ${r.name} (${r.bank}) à <strong style="color:var(--green)">${r.rate}%</strong></div>`;});
    }

    if(unusedWeb.length>0){
      const grouped={};
      unusedWeb.forEach(r=>{if(!grouped[r.bank])grouped[r.bank]={rates:[],maxRate:0};grouped[r.bank].rates.push(r);grouped[r.bank].maxRate=Math.max(grouped[r.bank].maxRate,r.rate);});
      html+=`<div style="margin-top:6px;font-size:10px;color:var(--text-dim)">❌ <strong>${unusedWeb.length} taux web non retenus</strong> (inférieurs à vos contrats) :</div>`;
      Object.entries(grouped).forEach(([bank,g])=>{
        html+=`<div style="font-size:10px;padding:2px 0;color:var(--text-muted)">  ${bank} : max ${g.maxRate}%${g.maxRate<bestConfirmedRate?' (< vos confirmés '+bestConfirmedRate+'%)':''} — ${g.rates.length} taux</div>`;
      });
    }

    if(bestWebRate<=bestConfirmedRate&&usedWeb.length===0){
      html+=`<div style="margin-top:8px;padding:8px;background:rgba(6,214,160,0.08);border-radius:4px;font-size:10px;color:var(--green)">
        💡 <strong>Bonne nouvelle :</strong> vos taux négociés (max ${bestConfirmedRate}%) sont meilleurs que tous les taux web (max ${bestWebRate}%). Pas d'opportunité supplémentaire via les taux indicatifs.
      </div>`;
    }
  ...
  html+=`<div style="border:1px solid var(--border);border-radius:var(--radius-sm);overflow:hidden"><div style="max-height:400px;overflow-y:auto"><table style="width:100%;border-collapse:collapse;font-size:11px"><thead style="position:sticky;top:0;z-index:1"><tr style="background:var(--bg-elevated);border-bottom:1px solid var(--border)">
    ...
    <th style="padding:8px 6px;text-align:right;color:var(--text-muted);font-weight:500">Gain NET/an</th>
    <th style="padding:8px 6px;text-align:center;color:var(--text-muted);font-weight:500">Seuil</th>
  ...
  depositAnalysis.forEach(d=>{
    ...
    html+=`<tr style="border-bottom:1px solid var(--border)" onmouseover="this.style.background='var(--bg-elevated)'" onmouseout="this.style.background=''">
      ...
      <td style="padding:8px 6px;text-align:center">${d.bestAlt?'<span style="font-family:var(--mono);font-weight:700;color:var(--cyan)">'+d.bestAlt.rate+'%</span>'+ab+'<div style="font-size:9px;color:var(--text-dim)">'+d.bestAlt.name+'</div>':'<span style="color:var(--green)">✨</span>'}</td>
      ...
      <td style="padding:8px 6px;text-align:center;font-size:9px;color:var(--text-dim)">${d.dynamicThreshold.toFixed(2)}%<div style="font-size:8px">${sp}pp</div></td>
...
function renderOptimizerDashboard() {
  ...
  const deps=r.deposits||[];const sl=r.rateSource==='all'?'🔍 tous':'✅ confirmés';const al=r.algorithmVersion==='v2'?'v2':'v1';
...
async function getAIOptimizerSummary(analysis) {
  ...
  const src=_optimizerRateSource==='confirmed'?'taux confirmés':'tous les taux (confirmés + '+_webCount+' web)';
  const webNote=_optimizerRateSource==='all'&&_webCount>0?`\n\nTAUX WEB ÉVALUÉS (${_webCount}):\n${(_webRatesSummary||[]).map(r=>`• ${r.name} (${r.bank}) ${r.rate}% — ${r.usedAsAlt?'✅ retenu comme alternative':'❌ inférieur aux contrats'}`).join('\n')}`:'';

  const dText=depositAnalysis.map(d=>{let l=`• ${d.name} (${d.bankName}) | ${d.amount}€ à ${d.rate}%`;if(d.currentPeriodRate!==d.rate)l+=` (palier: ${d.currentPeriodRate}%)`;l+=` | ${d.remainingMonths}m`;if(d.bestAlt){l+=` | Cible: ${d.bestAlt.rate}% (${d.bestAlt.name})${d.bestAlt.isScanned?' [web]':''}`;l+=` | Brut: +${d.grossGainAnnual}€`;if(d.exitPenaltyCost>0)l+=` | Pénalité: -${d.exitPenaltyCost}€`;l+=` | NET: ${d.netGainAnnual>0?'+':''}${d.netGainAnnual}€/an`;}l+=` → ${d.recommendation}`;return l;}).join('\n');
  ...
  const prompt=`Directeur financier. Optimisation v2. Source: ${src}.

**AVANT:** ${formatNumber(totalInvested)}€ à ${weightedRate.toFixed(2)}% = +${formatNumber(totalInterestPerYear)}€/an
**APRÈS:** ${optimizedRate.toFixed(2)}% = +${formatNumber(optimizedInterest)}€/an | NET: ${totalNetGain>0?'+':''}${formatNumber(totalNetGain)}€/an
Brut: +${formatNumber(totalGrossGain)}€ | Coûts: -${formatNumber(totalExitCost+totalNoticeCost)}€${webNote}
...
- **AVANT → APRÈS (NET +Z€)**
- Chaque arbitrage: 🔄 **[Nom]** → **[cible]** = **NET +Z€/an** ⚠️ [conditions]
- Si taux web utilisé: préciser "(indicatif web, à confirmer)"
- Si aucun taux web retenu: expliquer pourquoi (vos taux négociés sont meilleurs)
- ✅ **N contrats** leaders
- Max 180 mots`;
