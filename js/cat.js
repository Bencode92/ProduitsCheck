...
function renderCAT(container) {
  ...
  container.innerHTML = `
    ...
    ${timeline.length>0?`<div class="section"><div class="section-header"><div class="section-title"><span class="dot" style="background:var(--orange)"></span>Échéancier</div><span style="font-size:10px;color:var(--text-dim)">clic sur un mois → placements et meilleure offre de renouvellement</span></div><div class="cat-timeline">${timeline.map(m=>{const key=m.month.replace(/[^0-9]/g,'');const soon=_timelineDaysLeft(m)<=60;return `<div class="cat-timeline-item" style="cursor:pointer;user-select:none${soon?';border-left:3px solid var(--orange);padding-left:6px':''}" onclick="toggleTimelineMonth('${key}')" title="Voir les placements"><div class="cat-timeline-month">${m.month}${soon?' <span style="color:var(--orange);font-size:9px">⏰ '+_timelineDaysLeft(m)+'j</span>':''}</div><div class="cat-timeline-bar" style="width:${Math.min(100,(m.total/Math.max(stats.totalInvested,1))*300)}%"><span class="cat-timeline-amount">${formatNumber(m.total)}€</span></div><div class="cat-timeline-count">${m.deposits.length} placement${m.deposits.length>1?'s':''} <span id="tl-arrow-${key}" style="color:var(--text-dim);font-size:9px">▸</span></div></div><div id="tl-detail-${key}" style="display:none">${_renderTimelineDetail(m)}</div>`;}).join('')}</div></div>`:''}`;
}
...
// ─── Échéancier : détail par mois + meilleure offre de renouvellement ───────
function _timelineDaysLeft(m){const now=new Date();return Math.min(...m.deposits.map(d=>Math.round((new Date(d.maturityDate)-now)/864e5)));}
function _timelineCurrentRate(d){const today=new Date().toISOString().split('T')[0];if(d.rateSchedule&&d.rateSchedule.length){const cp=d.rateSchedule.find(s=>s.from<=today&&s.to>=today);if(cp)return parseFloat(cp.rate)||0;}return parseFloat(d.rate)||0;}
function _timelineBestOffer(durationMonths){const rates=(catManager.rates?.rates||[]).filter(r=>r.source!=='web scan'&&r.productType!=='parts-sociales'&&!(typeof _isRateExpired==='function'&&_isRateExpired(r))&&!/transition/i.test(r.productName||''));if(!rates.length)return null;const dm=parseInt(durationMonths,10)||12;let pool=rates.filter(r=>parseInt(r.durationMonths,10)===dm);if(!pool.length){/* durée la plus proche */const near=rates.reduce((b,r)=>Math.abs(r.durationMonths-dm)<Math.abs(b.durationMonths-dm)?r:b,rates[0]);pool=rates.filter(r=>r.durationMonths===near.durationMonths);}return pool.reduce((b,r)=>r.rate>(b?.rate||0)?r:b,null);}
function _renderTimelineDetail(m){const now=new Date();const rows=m.deposits.map(d=>{const days=Math.round((new Date(d.maturityDate)-now)/864e5);const cur=_timelineCurrentRate(d);const best=_timelineBestOffer(d.durationMonths);const bestTxt=best?`${best.bankName} ${best.productName?best.productName.replace(/^CAT\s+/i,''):best.durationMonths+'m'} · <strong style="color:${best.rate>cur?'var(--green)':'var(--text-bright)'}">${best.rate}%</strong> ${best.rate>cur?'<span style="color:var(--green)">(+'+(best.rate-cur).toFixed(2)+' pt)</span>':best.rate<cur?'<span style="color:var(--orange)">('+(best.rate-cur).toFixed(2)+' pt)</span>':''}`:'<span style="color:var(--text-dim)">aucune offre confirmée</span>';return `<tr style="border-bottom:1px solid var(--border)"><td style="padding:6px 8px"><strong style="color:var(--text-bright)">${d.productName||'CAT'}</strong><div style="font-size:9px;color:var(--text-dim)">${d.bankName||''}${d.entityName?' · '+d.entityName:''} · ${d.durationMonths||'?'} m</div>${d.reviewDate?'<div style="font-size:9px;color:var(--orange)">📌 décision le '+formatDate(d.reviewDate)+(d.reviewNote?' — '+d.reviewNote:'')+'</div>':''}</td><td style="padding:6px 8px;text-align:right;font-family:var(--mono)">${formatNumber(parseFloat(d.amount)||0)}€</td><td style="padding:6px 8px;text-align:right;font-family:var(--mono)">${cur}%${d.rateSchedule&&d.rateSchedule.length?' <span style="font-size:9px;color:var(--text-dim)">(TRAAB '+d.rate+'%)</span>':''}</td><td style="padding:6px 8px;text-align:right;white-space:nowrap">${formatDate(d.maturityDate)}<div style="font-size:9px;color:${days<=60?'var(--orange)':'var(--text-dim)'}">${days<0?'échu':'J-'+days}</div></td><td style="padding:6px 8px;font-size:11px">${bestTxt}</td></tr>`;}).join('');const wRate=m.deposits.reduce((s,d)=>s+(parseFloat(d.amount)||0)*_timelineCurrentRate(d),0)/Math.max(m.total,1);return `<div style="margin:4px 0 10px 0;background:var(--bg-elevated);border:1px solid var(--border);border-radius:var(--radius-sm);overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:11px;min-width:640px"><thead><tr style="border-bottom:1px solid var(--border)"><th style="text-align:left;padding:6px 8px;color:var(--text-muted);font-weight:600">Placement</th><th style="text-align:right;padding:6px 8px;color:var(--text-muted);font-weight:600">Montant</th><th style="text-align:right;padding:6px 8px;color:var(--text-muted);font-weight:600">Taux actuel</th><th style="text-align:right;padding:6px 8px;color:var(--text-muted);font-weight:600">Échéance</th><th style="text-align:left;padding:6px 8px;color:var(--text-muted);font-weight:600">Meilleure offre à durée équivalente</th></tr></thead><tbody>${rows}</tbody></table><div style="padding:6px 8px;font-size:10px;color:var(--text-dim)">Taux moyen pondéré du mois : <strong>${wRate.toFixed(2)}%</strong> · offres = taux confirmés non périmés, hors Transition ; « durée équivalente » = même durée que le placement (ou la plus proche)</div></div>`;}
function toggleTimelineMonth(key){const el=document.getElementById('tl-detail-'+key);const ar=document.getElementById('tl-arrow-'+key);if(!el)return;const open=el.style.display==='none';el.style.display=open?'block':'none';if(ar)ar.textContent=open?'▾':'▸';}
